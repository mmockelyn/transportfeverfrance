...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = '如果您的物品庫隱私設定為公開，社群成員便能透過此主題發送交易提案給您。';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>您的物品庫隱私設定為公開</b>。這代表社群成員能透過此主題發送交易提案給您。如果您的物品庫隱私設定並非公開，便無法接收交易提案。';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>您的物品庫隱私設定<u>並非</u>公開</b>。這代表社群成員無法透過此主題發送交易提案給您。如果您的物品庫隱私設定為公開，社群成員便能透過此主題發送交易提案給您。';
			...
			strMessage += '當您不想再接收交易提案或已完成交易的話，您可以在這裡關閉主題並禁止透過此處發送交易提案。';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( '建立主題時發生錯誤：' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( '訂閱討論區', '訂閱此討論區時發生錯誤。', '請稍後再試一次。' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( '取消訂閱討論區', '取消訂閱此討論區時發生錯誤。', '請稍後再試一次。' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
{
	ShowConfirmDialog('永久刪除 ',
		'您確定要永久刪除此主題嗎？這個動作無法復原。',
		'永久刪除'
	).done( function() {
...
{
	ShowConfirmDialog('清除可封鎖內容標記 ',
		'您確定要重設這個主題的可封鎖內容標記嗎？此動作無法復原。',
		'清除可封鎖內容標記'
	).done( function() {
...
{
	ShowConfirmDialog('封鎖留言者 ',
	'您確定要封鎖對此主題留言超過 5 次的使用者嗎？此舉將很難復原。',
	'封鎖留言者'
).done( function() {
...
{
	ShowConfirmDialog('標示為垃圾訊息 ',
		'此舉將刪除選取的討論串，並於討論區封鎖該作者。您確定要這麼做嗎？',
		'刪除垃圾訊息'
	).done( function() {
...
{
	ShowConfirmDialog( '關閉這個主題', '將此主題標記為「已關閉」將會禁止任何的回覆或交易提案。一旦關閉後，您將無法對其進行任何操作。', '關閉這個主題')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( '檢舉', '感謝您的檢舉', '您的檢舉已交由板務審查。若我們採取行動，將會通知您。' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', '主題修改失敗。錯誤訊息：' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', '修改主題失敗，請稍後再試一次。' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( '訂閱討論串', '您已訂閱此討論串。', '當有人回覆此討論串時，您將會收到通知。' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, '訂閱討論串', '很抱歉，我們無法訂閱此討論串。請稍後再試。');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( '取消訂閱討論串', '您已取消訂閱此討論串。', '您不會再收到來自此討論串的通知。' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, '取消訂閱討論串', '很抱歉，我們無法取消訂閱此討論串。請稍後再試。');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = '所有和此主題有關的回報已標記為已處理。<br>尚待處理回報數：' + cReportsResolved;
				else
					strMessage = '所有和此主題有關的回報已經被標記為已處理。';

				ShowForumSuccessDialogWithDetailTitle( '處理被檢舉的文章', '回報已處理。', strMessage );
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', '您確定您要刪除這篇留言嗎？', '刪除' )
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = '合併主題將使一個或多個主題中的所有留言轉移到另一個主題。如果目標主題中已經有其他留言，新加入的留言將與原有的留言一同按照時間順序重新排列。此動作一經執行不可復原。<br><br>';
	strMessage += rgForumTopics.length > 1 ? '挑選一個最近觀看過的主題，並將其與選擇的主題合併到：' : '選擇一個最近觀看過的主題，並將其與此主題合併到：';
	...
	var strMoreInstructions = '如果這裡沒有列出你想合併的主題，你可以在新的視窗<%1$s>尋找你要的主題<%2$s>。這個清單會在你返回的時候自動更新。'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, '將所選的主題併入較舊的主題' );
	...
	var Modal = ShowConfirmDialog( '合併主題', $DialogContent, '合併主題' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( '復原刪除', '所選的主題已經復原了。' );
			else
				ShowAlertDialog( '刪除', '已刪除所選擇的主題。' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( '解除鎖定', '所選的主題已經解鎖了。' );
			else
				ShowAlertDialog( '鎖定', '所選的主題已經鎖定了。' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( '移動...', '所選的主題已經移動到新的討論區了。' );
		});
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('永久刪除 ',
		...
		.done( function( data ) {
			ShowAlertDialog( '永久刪除', '選取的主題已被永久刪除。' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( '標示為垃圾訊息', '選取的主題已被刪除而作者也被封鎖。' );
		});
});
}
...
{
	ShowConfirmDialog('封鎖留言者 ',
	'此舉將對在所選討論串中留言超過特定次數的使用者施加社群封鎖（或標為被劫持）。您確定要這麼做嗎？',
	'封鎖留言者'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( '封鎖留言者', '對所選主題留言次數超過上述門檻的使用者已被封鎖。' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( '錯誤', '在處理您的所有或部分要求時發生錯誤，請確認您得到了預期的結果。' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( '警告使用者', $WaitElem, '警告使用者' );
	} else {
		var Modal = ShowConfirmDialog( '封鎖使用者', $WaitElem, '封鎖使用者' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( '封鎖使用者', '您必須至少選擇一個項目。該使使用者尚未被封鎖。' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( '封鎖使用者', ban_response.message ? ban_response.message : '該使用者的發文與編輯權限皆已被撤銷。' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( '封鎖使用者', ban_response.message ? ban_response.message : '您沒有封鎖使用者的權限，請確認您的帳戶權限或是稍後再試一次。' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( '警告使用者', ban_response.message ? ban_response.message : '已成功警告發表該文章的使用者。' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( '警告使用者', ban_response.message ? ban_response.message : '警告使用者時發生錯誤，請稍候再試。' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( '封鎖使用者', '您沒有封鎖使用者的權限，請確認您的帳戶權限或是稍後再試一次。' );
		}
		else {
			ShowAlertDialog( '警告使用者', '警告使用者時發生錯誤，請稍候再試。' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + '請輸入您希望私下傳送給此使用者的訊息：' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*私人訊息將自動引用此篇文章。</div>';

	var $modal = ShowConfirmDialog( '私人訊息', $strDialogHTML, '傳送私人訊息' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( '私人訊息', response.message );
				}
				else
				{
					ShowAlertDialog( '錯誤', '傳送私人訊息時發生錯誤，請稍後再試。' + '(' + response.success + ')' );
				}
			});
	} );
