...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Șterge discuţia ',
				'Sigur vrei să ştergi această discuţie? ' +
				'Această acţiune nu poate fi revocată de către un moderator.',
				'Șterge discuţia'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Marchează ca spam ',
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Raportează', 'Îţi mulţumim pentru sesizarea făcută', 'Raportarea ta a fost trimisă către moderatori pentru examinare. Te vom înștiința dacă se vor lua măsuri.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Discuţia nu a putut fi modificată. Te rugăm să încerci din nou mai târziu.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Abonează-te la discuţie', 'Te-ai abonat la această discuţie.', 'Vei primi o notificare de fiecare dată când cineva răspunde în această discuţie.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Abonează-te la discuţie', 'Ne cerem scuze, dar nu te putem abona la această discuţie. Te rugăm să încerci din nou mai târziu.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Dezabonează-te de la discuţie', 'Te-ai dezabonat de la această discuţie.', 'Nu vei mai primi notificări pentru răspunsurile noi din această discuţie.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Dezabonează-te de la discuţie', 'Ne cerem scuze, dar nu te-am putut dezabona de la această discuţie. Te rugăm să încerci din nou mai târziu.');
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Eşti sigur că vrei să ştergi acest mesaj?', 'Șterge' )
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Recuperează', 'Subiectele selectate au fost restaurate.' );
			else
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Marchează ca spam', 'Subiectele selectate au fost șterse, iar creatorii au fost banați.' );
		});
});
}
