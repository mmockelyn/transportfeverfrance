...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Se o teu inventário estiver com visibilidade Pública, membros da comunidade poderão enviar-te propostas de troca a partir deste tópico.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>A visibilidade do teu inventário está definida como Pública</b>. Isto significa que membros da comunidade podem enviar-te propostas de troca a partir deste tópico. Se o teu inventário estiver com outra visibilidade que não seja Pública, não poderás receber propostas.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>A visibilidade do teu inventário <u>não é</u> Pública</b>. Isto significa que membros da comunidade não podem enviar-te propostas de troca a partir deste tópico. Se o teu inventário estiver com visibilidade Pública, poderás receber propostas.';
			...
			strMessage += 'Quando não quiseres receber mais propostas de troca ou se já completaste a troca, podes fechar este tópico e desativar propostas de troca vindas daqui.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Ocorreu um erro ao criar um novo tópico:' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Subscrever fórum', 'Houve um erro ao subscrever este fórum.', 'Por favor, tenta novamente mais tarde.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Cancelar a Subscrição', 'Houve um erro ao anular a subscrição deste fórum.', 'Por favor, tenta novamente mais tarde.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Eliminar tópico ',
				'Tens a certeza de que pretendes eliminar este tópico? ' +
				'Esta ação só pode ser anulada por um moderador.',
				'Eliminar tópico'
			).done( function() {
					Topic.Delete();
				});
...
function Forum_PurgeTopic( gidTopic )
{
	ShowConfirmDialog('Eliminar permanentemente ',
		'De certeza que queres eliminar permanentemente este tópico? Isto será irreversível.',
...
{
	ShowConfirmDialog('Remover sinalização de conteúdo impróprio ',
		'Tens a certeza de que queres remover a sinalização de conteúdo impróprio deste tópico? Esta ação é irreversível.',
		'Remover sinalização de conteúdo impróprio'
	).done( function() {
...
{
	ShowConfirmDialog('Banir autores de comentários ',
	'Tens a certeza de que queres banir os utilizadores que comentaram mais de 5 vezes neste tópico? Esta ação será difícil de reverter.',
	'Banir autores de comentários'
).done( function() {
...
function Forum_MarkSpam( gidTopic )
{
	ShowConfirmDialog('Marcar como spam ',
		'Isto irá eliminar os tópicos selecionados e banir os autores do fórum. Tens a certeza de que queres fazer isto?',
		'Eliminar spam'
	).done( function() {
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						'Fundir tópicos',
						'Os tópicos selecionados foram fundidos num só tópico.',
						'Ir para o tópico fundido',
...
{
	ShowConfirmDialog( 'Fechar este tópico', 'Marcar este tópico como "Fechado" irá impedir que qualquer resposta ou proposta de troca seja deixada nele. Uma vez fechado, não será possível interagir mais com o tópico.', 'Fechar este tópico')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Denunciar', 'Agradecemos a tua denúncia', 'A tua denúncia foi enviada para ser analisada pelos moderadores. Receberás uma notificação se tomarmos medidas.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Não foi possível modificar o tópico. Mensagem de erro:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Ocorreu um erro ao modificar o tópico. Por favor, tenta de novo mais tarde.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Subscrever discussão', 'Subscreveste esta discussão.', 'Receberás uma notificação de comentário quando alguém responder a esta discussão.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Subscrever discussão', 'Lamentamos, mas não foi possível fazer a subscrição a esta discussão. Por favor, tenta de novo mais tarde.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Anular subscrição da discussão', 'A tua subscrição a esta discussão foi anulada.', 'Não irás receber mais notificações de comentários desta discussão.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Anular subscrição da discussão', 'Lamentamos, mas não foi possível anular a subscrição a esta discussão. Por favor, tenta de novo mais tarde.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Todas as denúncias associadas a este tópico foram \'resolvidas\'.<br>Número de denúncias que tinham de ser resolvidas:' + cReportsResolved;
				else
					strMessage = 'Todas as denúncias associadas a este tópico já foram marcadas como resolvidas.';

				ShowForumSuccessDialogWithDetailTitle( 'Resolver posts denunciados', 'Denúncias resolvidas.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Histórico de posts', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Tens a certeza de que pretendes eliminar este post?', 'Eliminar' )
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'Fundir os tópicos irá mover todos os posts de um ou mais tópicos para outro tópico. Se já existem posts no tópico-alvo, os posts novos serão intercalados conforme a data de postagem. Esta ação não poderá ser anulada.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Seleciona um tópico recentemente visto para com o qual fundir estes tópicos:' : 'Seleciona um tópico recentemente visto para com o qual fundir este tópico:';
	...
	var strMoreInstructions = 'Se o tópico com o qual gostarias de fundir não está aqui, <%1$s>procura pelo tópico que queres<%2$s> numa nova janela. Esta lista será atualizada automaticamente quando voltares.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Fundir os tópicos selecionados no tópico mais antigo' );
	...
	var Modal = ShowConfirmDialog( 'Fundir tópicos', $DialogContent, 'Fundir tópicos' );
	...
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				...
				ShowAlertDialog( 'Eliminar', 'Os tópicos selecionados foram eliminados.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Desbloquear', 'Os tópicos selecionados foram desbloqueados.' );
			else
				ShowAlertDialog( 'Bloquear', 'Os tópicos selecionados foram bloqueados.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Mover...', 'Os tópicos selecionados foram movidos para o novo fórum.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Fundir tópicos',
					'Os tópicos selecionados foram fundidos num só tópico.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Eliminar permanentemente ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Eliminar permanentemente', 'Os tópicos selecionados foram eliminados permanentemente.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Marcar como spam', 'Os tópicos selecionados foram eliminados e os criadores foram banidos.' );
		});
});
}
...
{
	ShowConfirmDialog('Banir autores de comentários ',
	'Isto irá banir da comunidade (ou marcar como "conta roubada") os utilizadores que comentaram nos tópicos selecionados mais do que um certo número de vezes. Tens a certeza de que é isto que pretendes fazer?',
	'Banir autores de comentários'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Banir autores de comentários', 'Os utilizadores que ultrapassaram o limite de comentários nos tópicos selecionados foram banidos.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Erro', 'Ocorreu um erro ao processar todo ou parte do teu pedido. Verifica se os resultados foram os esperados.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Avisar utilizador', $WaitElem, 'Avisar utilizador' );
	} else {
		var Modal = ShowConfirmDialog( 'Banir Utilizador', $WaitElem, 'Banir Utilizador' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Banir Utilizador', 'Pelo menos uma opção tem de ser selecionada. O utilizador não foi banido.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Banir Utilizador', ban_response.message ? ban_response.message : 'Os privilégios de publicação e edição do utilizador foram revogados.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Banir Utilizador', ban_response.message ? ban_response.message : 'Podes não ter permissão para banir utilizadores. Por favor, verifica as permissões da tua conta ou tenta de novo mais tarde.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Avisar utilizador', ban_response.message ? ban_response.message : 'Este utilizador foi avisado devido ao post que ele fez.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Avisar utilizador', ban_response.message ? ban_response.message : 'Ocorreu um erro ao avisar este utilizador, tenta de novo mais tarde.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Banir Utilizador', 'Podes não ter permissão para banir utilizadores. Por favor, verifica as permissões da tua conta ou tenta de novo mais tarde.' );
		}
		else {
			ShowAlertDialog( 'Avisar utilizador', 'Ocorreu um erro ao avisar este utilizador, tenta de novo mais tarde.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Introduz a mensagem que gostarias de enviar em privado para este utilizador:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Este post será mencionado na tua mensagem privada automaticamente.</div>';
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Erro', 'Ocorreu um erro ao enviar esta mensagem privada. Tenta de novo mais tarde.' + '(' + response.success + ')' );
				}
			});
	} );
