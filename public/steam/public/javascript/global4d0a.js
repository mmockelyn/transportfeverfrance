	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Възможно е съдържанието да не е подходящо за всички възрасти', data['html'], 'Преглед на съдържанието' );
				...
				{
					ShowAlertDialog('Възможно е съдържанието да не е подходящо за всички възрасти', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Преглед на предупреждение относно съдържание за възрастни' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Премахване на неяснотата само за това съдържание' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Замъгляване на това съдържание' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Възможно е съдържанието да не е подходящо за всички аудитории'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Маркиране на спам', 'Това ще даде обществена забрана на автора. Добавете този коментар като обучителни данни. Така ще можем автоматично да маркираме тези видове коментари. Както и да изчистим всички коментари на автора. Сигурни ли сте, че искате да сторите това?', 'Маркиране на спам' );
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Изчистване на флага относно съдържание за забрана', 'Това ще изчисти флага относно проверката на съдържанието за забрана върху този коментар и не може да бъде отменено. Сигурни ли сте, че искате да сторите това?', 'Изчистване на флага относно съдържание за забрана' );
		dialog.done( function() {
			thread.m_bLoading = true;
