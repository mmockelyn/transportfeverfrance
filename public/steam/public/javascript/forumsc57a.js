...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = '보관함 공개 설정이 공개로 되어 있다면 이 주제에서 커뮤니티 구성원들이 나에게 거래 제안을 보낼 수 있습니다.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>보관함 공개 설정이 공개로 되어 있습니다</b>. 커뮤니티 구성원들이 이 주제에서 나에게 거래 제안을 보낼 수 있습니다. 보관함 공개 설정이 공개 이외의 다른 것으로 되어 있다면 커뮤니티 거래 제안을 받을 수 없습니다.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>보관함 공개 설정이 공개로 되어 있지 <u>않습니다</u></b>. 커뮤니티 구성원들이 이 주제에서 나에게 거래 제안을 보낼 수 없습니다. 보관함 공개 설정이 공개로 되어 있다면 이 주제에서 커뮤니티 구성원들이 나에게 거래 제안을 보낼 수 있습니다.';
			...
			strMessage += '당신이 받는 거래 제안을 완료하거나 거래를 완료하면, 귀하는 여기에서 이 항목을 닫고 거래 제공을 거부하실수 있습니다.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( '새 주제를 생성하는 중에 오류가 발생하였습니다.' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( '포럼 구독하기', '포럼 구독을 진행하는 중에 오류가 발생했습니다.', '잠시 후에 다시 시도하십시오.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( '포럼 구독 취소하기', '포럼 구독 해지를 진행하는 중에 오류가 발생했습니다.', '잠시 후에 다시 시도하십시오.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('글 삭제 ',
				'이 글타래를 삭제하겠습니까? ' +
				'이 조치는 모더레이터에 의해서만 취소됩니다.',
				'글 삭제'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('영구적으로 삭제 ',
		'정말로 이 토론을 영구적으로 삭제할까요? 삭제된 토론은 복구할 수 없습니다.',
		'영구적으로 삭제'
	).done( function() {
...
{
	ShowConfirmDialog('차단 가능 콘텐츠의 깃발 표시 제거 ',
		'이 주제에서 차단 가능 콘텐츠의 깃발 표시를 제거할까요? 이 작업은 취소할 수 없습니다.',
		'차단 가능 콘텐츠의 깃발 표시 제거'
	).done( function() {
...
{
	ShowConfirmDialog('작성자 차단 ',
	'이 주제에 대해 5회 넘게 글을 남긴 작성자를 차단할까요? 차단 실행 시 취소가 어렵습니다.',
	'작성자 차단'
).done( function() {
...
{
	ShowConfirmDialog('스팸으로 표시 ',
		'선택한 글타래를 삭제하고 게시자를 포럼에서 차단합니다. 계속 진행할까요?',
		'스팸 삭제'
	).done( function() {
...
{
	ShowConfirmDialog( '토론 닫기', '주제를 "닫힘"으로 표시하면 더 이상의 댓글이나 거래 제안이 달리지 않습니다. 일단 한 번 닫히고 나면, 그에 대해 추가적인 작업은 불가능해집니다.', '토론 닫기')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( '신고', '신고해주셔서 감사합니다', '신고하신 내용은 검토를 위해 모더레이터에게 전달되었습니다. 조치가 내려지면 알려드리겠습니다.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', '주제를 수정하지 못했습니다. 오류 메시지:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', '글을 수정할 수 없습니다. 다시 시도해 보십시오.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( '토론 구독', '이 토론을 구독하였습니다.', '이 토론에 새로운 댓글이 달릴 때마다 댓글 알림을 받게 됩니다.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, '토론 구독', '이 토론을 구독할 수 없습니다. 잠시 후 다시 시도해 보세요.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( '토론 구독 취소', '이 토론의 구독을 취소하였습니다.', '더 이상 이 토론의 댓글 알림을 받지 않게 됩니다.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, '토론 구독 취소', '이 토론의 구독을 취소할 수 없습니다. 잠시 후 다시 시도하세요.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = '이 토론에 관련된 모든 신고가 해결된 것으로 표시되었습니다.<br>해결한 신고 수:' + cReportsResolved;
				else
					strMessage = '이 토론에 관련된 모든 신고가 이미 완료된 것으로 표시되었습니다.';

				ShowForumSuccessDialogWithDetailTitle( '신고된 게시물 해결', '신고가 해결되었습니다.', strMessage );
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', '이 게시글을 삭제하겠습니까?', '삭제' )
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = '주제들을 서로 합칠 경우 주제들에 달린 댓글들이 다른 주제로 옮겨지게 됩니다. 합칠 주제에 달린 댓글들은 원래 주제에 추가되어 시간 순서대로 정렬될 것입니다. 이 동작은 취소할 수 없습니다.<br><br>';
	strMessage += rgForumTopics.length > 1 ? '최근에 본 주제들을 선택하여 하나의 주제로 합치십시오' : '이 토론을 합칠 최근에 본 토론을 하나 고르세요:';
	...
	var strMoreInstructions = '만약 합치고자 하는 주제가 여기에 나열되지 않는다면, 새 창에서 <%1$s>원하는 토픽을 검색할 수 있습니다<%2$s>. 그 후 이 리스트로 돌아오면 리스트가 자동으로 업데이트 됩니다.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( '인용 사항 남겨두기: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, '선택한 주제들을 제일 오래된 주제로 합치기' );
	...
	var Modal = ShowConfirmDialog( '주제 합치기', $DialogContent, '주제 합치기' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( '삭제 취소', '선택된 주제들이 복구되었습니다.' );
			else
				ShowAlertDialog( '삭제', '선택된 주제들이 삭제되었습니다.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( '잠금 해제', '선택된 주제들이 잠금 해제되었습니다.' );
			else
				ShowAlertDialog( '잠금', '선택된 주제들이 잠금 처리 되었습니다.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( '이동...', '선택된 주제들이 새 포럼으로 이동되었습니다.' );
		});
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('영구적으로 삭제 ',
		...
		.done( function( data ) {
			ShowAlertDialog( '영구적으로 삭제', '선택된 주제가 영구적으로 삭제되었습니다.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( '스팸으로 표시', '선택한 토론을 삭제하고 게시자를 차단했습니다.' );
		});
});
}
...
{
	ShowConfirmDialog('작성자 차단 ',
	'선택된 글타래에 특정 횟수 이상으로 글을 남긴 작성자에게 커뮤니티 차단(또는 계정 탈취자 표식)을 적용합니다. 계속 진행할까요?',
	'작성자 차단'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( '작성자 차단', '선택된 주제에 대해 제한된 횟수보다 많은 글을 남긴 작성자가 차단 처리되었습니다.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( '오류', '모든 또는 일부 요청을 처리하는 중 오류가 발생했습니다. 기대한 결과가 나왔는지 검증해보시기 바랍니다.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( '사용자에게 경고', $WaitElem, '사용자에게 경고' );
	} else {
		var Modal = ShowConfirmDialog( '사용자 차단', $WaitElem, '사용자 차단' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( '사용자 차단', '적어도 하나의 옵션을 선택해야 합니다. 사용자를 차단하지 않았습니다.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( '사용자 차단', ban_response.message ? ban_response.message : '본 이용자의 글 게시와 편집의 권한이 철회되었습니다.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( '사용자 차단', ban_response.message ? ban_response.message : '귀하의 계정으로는 사용자 차단을 할 수 없을 수도 있습니다. 권한을 확인하시거나 다시 시도하십시오.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( '사용자에게 경고', ban_response.message ? ban_response.message : '사용자에게 게시물에 대한 경고를 보내는 데 성공했습니다.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( '사용자에게 경고', ban_response.message ? ban_response.message : '사용자에게 경고를 보내는 데 오류가 발생했습니다. 나중에 다시 시도해 주세요.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( '사용자 차단', '귀하의 계정으로는 사용자 차단을 할 수 없을 수도 있습니다. 권한을 확인하시거나 다시 시도하십시오.' );
		}
		else {
			ShowAlertDialog( '사용자에게 경고', '사용자에게 경고를 보내는 데 오류가 발생했습니다. 나중에 다시 시도해 주세요.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + '이 사용자에게 비공개로 보낼 메시지를 입력하세요:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">* 이 게시물은 귀하의 비공개 메시지에서 자동으로 참조합니다.</div>';

	var $modal = ShowConfirmDialog( '비공개 메시지', $strDialogHTML, '비공개 메시지 보내기' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( '비공개 메시지', response.message );
				}
				else
				{
					ShowAlertDialog( '오류', '비공개 메시지를 보내는 데 오류가 발생했습니다. 다시 시도해 주세요.' + '(' + response.success + ')' );
				}
			});
	} );
