	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('A tartalom esetleg nem minden korosztály számára megfelelő', data['html'], 'Tartalom megnézése' );
				...
				{
					ShowAlertDialog('A tartalom esetleg nem minden korosztály számára megfelelő', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Felnőtt tartalom figyelmeztetés megnézése' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Homályosítás eltávolítása csak e tartalomhoz' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'E tartalom homályosítása' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'A tartalom nem feltétlenül megfelelő minden közönségnek'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Spamnek jelölés', 'Ez közösségi kitiltást fog adni a szerzőnek, tanítóadatként eltárolja ezt a megjegyzést, hogy az ilyeneket automatikusan meg tudjuk jelölni, és törli a szerző összes megjegyzését. Biztosan ezt akarod tenni?', 'Spamnek jelölés' );
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						...
						{
							strError = 'Nem vagy jogosult frissíteni a téma válaszát.'
						}
