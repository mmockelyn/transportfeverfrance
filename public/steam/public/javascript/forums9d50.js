...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Wenn Ihre Inventarprivatsphäre auf "Öffentlich" gestellt ist, können Ihnen Communitymitglieder Handelsangebote senden.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>Ihre Inventarprivatsphäre ist auf "Öffentlich" gestellt</b>. Das heißt, dass Community-Mitgieder Ihnen Handelsangebote senden können. Wenn Ihre Privatsphäre auf irgendetwas anderem als öffentlich steht, können Sie keine Community-Handelsangebote erhalten.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>Ihre Inventarprivatsphäre ist <u>nicht</u> auf "Öffentlich" gestellt</b>. Das heißt, dass Communitymitglieder Ihnen keine Handelsangebote senden können. Wenn Ihre Privatsphäre auf öffentlich stehen würde, könnten Communitymitglieder Ihnen Handelsangebote senden.';
			...
			strMessage += 'Wenn Sie keine Handelsangebote mehr erhalten möchten oder den Handel abgeschlossen haben, können Sie dieses Thema schließen und Handelsangebote von hier unterbinden.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Beim Erstellen eines neuen Themas ist ein Fehler aufgetreten: ' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Forum abonnieren', 'Beim Abonnieren dieses Forums ist ein Fehler aufgetreten.', 'Bitte versuchen Sie es später erneut.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Forum deabonnieren', 'Beim Deabonnieren dieses Forums ist ein Fehler aufgetreten.', 'Bitte versuchen Sie es später erneut.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Thema löschen ',
				'Möchten Sie dieses Thema wirklich löschen? ' +
				'Diese Aktion kann nur durch einen Moderator rückgängig gemacht werden.',
				'Thema löschen'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Unwiderruflich löschen ',
		'Möchten Sie dieses Thema wirklich unwiderruflich löschen? Dies kann nicht rückgängig gemacht werden.',
...
{
	ShowConfirmDialog('Markierung für verbotene Inhalte entfernen ',
		'Möchten Sie die Markierung für verbotene Inhalte von diesem Thema wirklich entfernen? Dies kann nicht rückgängig gemacht werden.',
		'Markierung für verbotene Inhalte entfernen'
	).done( function() {
...
{
	ShowConfirmDialog('Kommentierende ausschließen ',
	'Möchten Sie wirklich die Nutzer ausschließen, die mehr als 5 Kommentare zu diesem Thema verfasst haben? Dies kann nur schwer rückgängig gemacht werden.',
	'Kommentierende ausschließen'
).done( function() {
...
{
	ShowConfirmDialog('Als Spam markieren ',
		'Hierdurch werden die ausgewählten Themen gelöscht und die Autoren aus dem Forum ausgeschlossen. Möchten Sie dies wirklich tun?',
		'Spam löschen'
	).done( function() {
			...
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						...
						'Die ausgewählten Themen wurden zusammengeführt.',
						'Zum zusammengeführten Thema',
...
{
	ShowConfirmDialog( 'Dieses Thema schließen', 'Durch das Markieren dieses Themas als "Geschlossen" wird verhindert, dass es mehr Antworten oder Handelsangebote geben wird. Sobald es geschlossen wurde, werden Sie keine Möglichkeit mehr dazu haben, es zu bearbeiten.', 'Dieses Thema schließen')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Melden', 'Vielen Dank für Ihre Meldung', 'Ihre Meldung wurde zur Überprüfung an die Moderatoren weitergeleitet. Wir werden Sie informieren, falls Maßnahmen ergriffen werden.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Thema konnte nicht bearbeitet werden. Fehlermeldung:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Thema konnte nicht bearbeitet werden. Bitte versuchen Sie es später erneut.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Diskussion abonnieren', 'Sie haben diese Diskussion abonniert.', 'Sie werden eine Benachrichtigung erhalten, sobald jemand auf diese Diskussion antwortet.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Diskussion abonnieren', 'Entschuldigung, diese Diskussion konnte nicht abonniert werden. Bitte versuchen Sie es später erneut.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Diskussion deabonnieren', 'Sie haben diese Diskussion deabonniert.', 'Sie werden keine weiteren Benachrichtigungen zu dieser Diskussion erhalten.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Diskussion deabonnieren', 'Entschuldigung, diese Diskussion konnte nicht deabonniert werden. Bitte versuchen Sie es später erneut.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Alle Meldungen zu diesem Thema wurden als "geklärt" markiert.<br>Anzahl der Meldungen, die geklärt werden mussten: ' + cReportsResolved;
				else
					strMessage = 'Alle Meldungen zu diesem Thema wurden bereits als "geklärt" markiert.';

				ShowForumSuccessDialogWithDetailTitle( 'Gemeldete Beiträge klären', 'Meldungen geklärt.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Beitragsverlauf', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Möchten Sie diesen Beitrag wirklich löschen?', 'Löschen' )
...
var g_rgRedirectTimeOptions = [
	{value: 0, label: 'Keine Weiterleitung' },
	{value: 1, label: '1 Stunde' },
	{value: 24, label: '1 Tag' },
	{value: 168, label: '1 Woche' },
	{value: 720, label: '1 Monat' },
	{value: -1, label: 'Dauerhaft' }
];
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'Durch das Zusammenführen von Themen werden alle Beiträge von einem oder mehreren Themen in ein anderes Thema verschoben. Wenn es bereits Beiträge im Zielthema gibt, werden die neuen Beiträge dort nach Verfassungsdatum eingeordnet. Dies kann nicht rückgängig gemacht werden.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Wählen Sie ein kürzlich angesehenes Thema aus, um es mit diesen Themen zusammenzuführen:' : 'Wählen Sie ein kürzlich angesehenes Thema aus, um es mit diesem zusammenzuführen:';
	...
	var strMoreInstructions = 'Falls das Zielthema hier nicht aufgeführt ist, können Sie in einem neuen Fenster <%1$s>das gewünschte Thema suchen<%2$s>. Diese Liste wird automatisch aktualisiert, wenn Sie zurückkehren.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Weiterleitung hinterlassen für: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Ausgewählte Themen im ältesten Thema zusammenführen' );
	...
	var Modal = ShowConfirmDialog( 'Themen zusammenführen', $DialogContent, 'Themen zusammenführen' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Wiederherstellen', 'Die ausgewählten Themen wurden wiederhergestellt.' );
			else
				ShowAlertDialog( 'Löschen', 'Die ausgewählten Themen wurden gelöscht.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Öffnen', 'Die ausgewählten Themen wurden geöffnet.' );
			else
				ShowAlertDialog( 'Schließen', 'Die ausgewählten Themen wurden geschlossen.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Verschieben …', 'Die ausgewählten Themen wurden in das neue Forum verschoben.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Themen zusammenführen',
					'Die ausgewählten Themen wurden zusammengeführt.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Unwiderruflich löschen ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Unwiderruflich löschen', 'Die ausgewählten Themen wurden unwiderruflich gelöscht.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Als Spam markieren', 'Die ausgewählten Themen wurden gelöscht und die Ersteller ausgeschlossen.' );
		});
});
}
...
{
	ShowConfirmDialog('Kommentierende ausschließen ',
	'Hierdurch werden Accounts von Nutzern, die zu den ausgewählten Themen mehr als eine bestimmte Anzahl Kommentare verfasst haben, aus der Community ausgeschlossen (oder als gestohlen markiert). Möchten Sie dies wirklich tun?',
	'Kommentierende ausschließen'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Kommentierende ausschließen', 'Die Benutzer, die zu den ausgewählten Themen zu viele Kommentare verfasst haben, wurden ausgeschlossen.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Fehler', 'Bei der Verarbeitung Ihrer Anfrage oder eines Teils davon ist ein Fehler aufgetreten. Bitte überprüfen Sie, ob sie das erwartete Ergebnis erhalten haben.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Nutzer verwarnen', $WaitElem, 'Nutzer verwarnen' );
	} else {
		var Modal = ShowConfirmDialog( 'Benutzer ausschließen', $WaitElem, 'Benutzer ausschließen' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Benutzer ausschließen', 'Mindestens eine Option muss ausgewählt werden. Der Benutzer wurde nicht ausgeschlossen.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Benutzer ausschließen', ban_response.message ? ban_response.message : 'Die Berechtigung, Beiträge zu veröffentlichen und zu bearbeiten, wurde dem Benutzer entzogen.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Benutzer ausschließen', ban_response.message ? ban_response.message : 'Sie verfügen eventuell nicht über die Berechtigung, Benutzer auszuschließen. Bitte überprüfen Sie Ihre Benutzerberechtigungen oder versuchen Sie es später erneut.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Nutzer verwarnen', ban_response.message ? ban_response.message : 'Der Nutzer wurde erfolgreich für seinen Beitrag verwarnt.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Nutzer verwarnen', ban_response.message ? ban_response.message : 'Beim Verwarnen des Nutzers ist ein Fehler aufgetreten, bitte versuchen Sie es später erneut.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Benutzer ausschließen', 'Sie verfügen eventuell nicht über die Berechtigung, Benutzer auszuschließen. Bitte überprüfen Sie Ihre Benutzerberechtigungen oder versuchen Sie es später erneut.' );
		}
		else {
			ShowAlertDialog( 'Nutzer verwarnen', 'Beim Verwarnen des Nutzers ist ein Fehler aufgetreten, bitte versuchen Sie es später erneut.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Bitte geben Sie die Nachricht ein, die Sie diesem Nutzer privat senden möchten:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Eine Kopie dieses Beitrags wird Ihrer privaten Nachricht automatisch beigefügt.</div>';

	var $modal = ShowConfirmDialog( 'Private Nachricht', $strDialogHTML, 'Private Nachricht senden' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Private Nachricht', response.message );
				}
				else
				{
					ShowAlertDialog( 'Fehler', 'Beim Senden dieser privaten Nachricht ist ein Fehler aufgetreten. Bitte versuchen Sie es erneut.' + '(' + response.success + ')' );
				}
			});
	} );
