...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		if ( this.m_rgCreateTopicFlags !== null && strForumType == 'Trading' )
		{
			...
			strMessage += 'Cuando tengas suficientes ofertas o hayas completado el intercambio, puedes cerrar este tema para no recibir más ofertas desde aquí.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Se ha producido un error al crear un nuevo tema: ' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Suscribirse al foro', 'Se ha producido un error al suscribirse a este foro.', 'Por favor, inténtalo de nuevo más tarde.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Anular suscripción al foro', 'Se ha producido un error al anular la suscripción a este foro.', 'Por favor, inténtalo de nuevo más tarde.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Marcar como spam', 'Se han eliminado los temas seleccionados y se ha bloqueado a sus creadores.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Error', 'Se ha producido un error mientras procesábamos toda o una parte de tu petición. Por favor, verifica que has obtenido los resultados esperados.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Advertir al usuario', ban_response.message ? ban_response.message : 'Se ha producido un error al intentar advertir a este usuario. Inténtalo de nuevo más tarde.' );
				});
			});
		}
		...
		if ( !bWarning ) {
			...
			ShowAlertDialog( 'Advertir al usuario', 'Se ha producido un error al intentar advertir a este usuario. Inténtalo de nuevo más tarde.' );
		}
	});
}
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Error', 'Se ha producido un error al enviar este mensaje privado. Inténtalo de nuevo.' + '(' + response.success + ')' );
				}
			});
	} );
