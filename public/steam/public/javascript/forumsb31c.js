...
var CForum = Class.create( {
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Suscribirse a foro', 'Se ha producido un error al suscribirse a este foro.', 'Por favor, inténtalo de nuevo más tarde.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Marcar como spam', 'Los temas seleccionados han sido eliminados y los creadores, bloqueados.' );
		});
});
}
