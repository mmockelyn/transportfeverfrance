	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Este conteúdo pode não ser apropriado para todas as idades', data['html'], 'Ver conteúdo' );
				...
				{
					ShowAlertDialog('Este conteúdo pode não ser apropriado para todas as idades', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Impedir que este conteúdo apareça desfocado' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Desfocar este conteúdo' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Marcar como spam', 'Isto irá banir o autor da comunidade, usar este comentário como referência para que possamos sinalizar este tipo de comentários automaticamente e remover todos os comentários do autor. Tens a certeza de que pretendes fazer isto?', 'Marcar como spam' );
	...
	DisplayError: function( elError, transport )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.error )
			...
			strMessage += 'Ocorreu um erro ao comunicar com a rede. Tenta de novo mais tarde.';
