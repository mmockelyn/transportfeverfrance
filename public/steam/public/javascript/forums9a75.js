...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Envanter gizliliğiniz Herkese Açık ise, topluluk üyeleri bu başlıktan size takas teklifi gönderebilirler.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>Envanter gizliliğiniz Herkese Açık olarak ayarlandı</b>. Bu, topluluk üyelerinin bu başlıktan size takas teklifi gönderebileceği anlamına geliyor. Envanter gizliliğiniz Herkese Açık\'tan başka bir şeye ayarlanırsa, Topluluk takas tekliflerini alamazsınız.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>Envanter gizliliğiniz Herkese Açık <u>değil</u></b>. Bu, topluluk üyelerinin bu başlıktan size takas teklifi gönderemeyeceği anlamına geliyor. Envanter gizliliğiniz Herkese Açık olarak ayarlanmış olsaydı, topluluk üyeleri size bu başlıktan takas teklifi gönderebilecekti.';
			...
			strMessage += 'Takas teklifi almayı bitirdiğinizde ya da takası tamamladığınızda, bu başlığı kapatabilir ve takas tekliflerini buradan devre dışı bırakabilirsiniz.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Yeni konu açılırken bir hata meydana geldi: ' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Foruma Abone Ol', 'Bu foruma abone olunurken bir hata meydana geldi.', 'Lütfen daha sonra tekrar deneyin.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Forum Aboneliğinden Çık', 'Bu forumun aboneliğinden çıkılırken bir hata meydana geldi.', 'Lütfen daha sonra tekrar deneyin.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Başlığı Sil ',
				'Bu başlığı silmek istediğinizden emin misiniz? ' +
				'Bu eylem sadece bir moderatör tarafından geri alınabilir.',
				'Başlığı Sil'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Kalıcı olarak Sil ',
		'Bu konuyu kalıcı olarak silmek istediğinden emin misin? Bu işlem geri alınamaz.',
		'Kalıcı olarak Sil'
	).done( function() {
...
{
	ShowConfirmDialog('Yasaklanabilir İçerik İşaretini Temizle ',
		'Bu başlığın yasaklanabilir içerik işaretini kaldırmak istediğinize emin misiniz? Bu geri alınamaz.',
		'Yasaklanabilir İçerik İşaretini Temizle'
	).done( function() {
...
{
	ShowConfirmDialog('Yorum Yapanları Yasakla ',
	'Bu başlıkta 5 kereden fazla yorum yapmış kullanıcıları yasaklamak istediğinizden emin misiniz? Bunu geri almak zor olacaktır.',
	'Yorum Yapanları Yasakla'
).done( function() {
...
{
	ShowConfirmDialog('Gereksiz Olarak İşaretle ',
		'Bu, seçili başlıkları silecek ve yazarları forumdan yasaklayacak. Bunu yapmak istediğinize emin misiniz?',
		'Gereksizleri Sil'
	).done( function() {
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						'Konuları Birleştir',
						'Seçilen konular tek bir başlık altında birleştirildi.',
						'Birleştirilmiş başlığa git',
						'Konu listesine dön'
					).done( function() {
...
{
	ShowConfirmDialog( 'Bu başlığı kapat', 'Bu konuyu "Kapandı" olarak işaretlemek, bu konuya daha fazla cevap yazılmasını veya takas teklifi gönderilmesini engelleyecektir. Bir kez kapatıldığında, konu üzerinde daha fazla değişiklik yapamayacaksınız.', 'Bu başlığı kapat')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Bildir', 'Bildiriniz için teşekkürler', 'Şikâyetiniz incelenmek üzere moderatörlere gönderildi. Eğer bir eylemde bulunulursa size haber vereceğiz.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Başlık değiştirilemedi. Hata mesajı:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Başlık düzenlenemedi. Lütfen daha sonra tekrar deneyin.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Başlığa abone ol', 'Bu başlığa abone oldunuz.', 'Birisi bu başlığa yanıt gönderdiğinde yorum bildirimleri alacaksınız.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Başlığa abone ol', 'Üzgünüz, şu anda bu başlığa abone olamıyoruz. Lütfen daha sonra tekrar deneyin.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Başlıktaki aboneliğimi kaldır', 'Bu başlığa olan aboneliğiniz kaldırıldı.', 'Artık bu başlığa ait yorum bildirimleri almayacaksınız.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Başlıktaki aboneliğimi kaldır', 'Üzgünüz, şu anda bu başlığa olan aboneliğinizi kaldıramıyoruz. Lütfen daha sonra tekrar deneyin.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Bu konu ile alakalı tüm bildiriler çözülmüş olarak işaretlendi.<br>Çözülmüş olarak işaretlenen bildiri sayısı:' + cReportsResolved;
				else
					strMessage = 'Bu konu ile alakalı tüm bildiriler çözülmüş olarak işaretlendi.';

				ShowForumSuccessDialogWithDetailTitle( 'Bildirilen iletileri gözden geçir', 'Bildiriler çözüldü.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'İleti geçmişi', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Bu yorumu silmek istediğinize emin misiniz?', 'Sil' )
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'Konuları birleştirmek, bir veya birden fazla konu altındaki tüm mesajları başka bir konu ile birleştirmektir. Eğer hedef konuda halihazırda mesajlar mevcutsa, yeni mesajlar gönderildikleri tarihlere göre sıralanacaktır. Bunun geri dönüşü yoktur.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Seçilmiş konuları birleştirmek için son zamanlarda görüntülenmiş bir konu seçin:' : 'Bu konuyu birleştirmek için son görüntülenen konulardan birini seçiniz:';
	...
	var strMoreInstructions = 'Birleştirmek istediğiniz başlık burada yoksa, yeni bir pencerede <%1$s>istediğiniz başlığa göz atabilirsiniz<%2$s>. Bu liste döndüğünüz zaman otomatik olarak güncellenmiş olacaktır.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Yönlendirme bırak: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Seçilmiş konuları en eski konu ile birleştir' );
	...
	var Modal = ShowConfirmDialog( 'Konuları Birleştir', $DialogContent, 'Konuları Birleştir' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Silineni Geri Al', 'Seçilmiş konular geri getirildi.' );
			else
				ShowAlertDialog( 'Sil', 'Seçilen konular silindi.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Kilidi Aç', 'Seçili konuların kilidi açıldı.' );
			else
				ShowAlertDialog( 'Kilitle', 'Seçili konular kilitlendi.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Taşı...', 'Seçili konular yeni foruma taşındı.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Konuları Birleştir',
					'Seçilen konular tek bir başlık altında birleştirildi.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Kalıcı olarak Sil ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Kalıcı olarak Sil', 'Seçili konular kalıcı olarak silindi.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Gereksiz Olarak İşaretle', 'Seçili konular silindi ve konu sahipleri yasaklandı.' );
		});
});
}
...
{
	ShowConfirmDialog('Yorum Yapanları Yasakla ',
	'Bu, seçilmiş başlıklarda belirli bir sayıdan daha fazla yorumu olan kullanıcılara topluluk yasağı verir veya kullanıcıları, hesapları çalınmış olarak işaretler. Yapmak istediğinizin bu olduğuna emin misiniz?',
	'Yorum Yapanları Yasakla'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Yorum Yapanları Yasakla', 'Seçilmiş başlıklarda belirlenen sayının üzerinde yorum yapan kullanıcılar yasaklandı.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Hata', 'İsteğinizin veya bir kısmının işlenmesinde bir hata meydana geldi. Lütfen istenilen sonuçlara ulaştığınızı doğrulayın.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Kullanıcıyı Uyar', $WaitElem, 'Kullanıcıyı Uyar' );
	} else {
		var Modal = ShowConfirmDialog( 'Kullanıcıyı Yasakla', $WaitElem, 'Kullanıcıyı Yasakla' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Kullanıcıyı Yasakla', 'En azından bir seçenek işaretlenmelidir. Kullanıcı yasaklanmadı.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Kullanıcıyı Yasakla', ban_response.message ? ban_response.message : 'Bu kullancının gönderme ve düzenleme hakları iptal edilmiştir.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Kullanıcıyı Yasakla', ban_response.message ? ban_response.message : 'Kullanıcıları yasaklama iznine sahip olmayabilirsiniz. Lütfen hesabınızın izinlerini doğrulayın ya da daha sonra tekrar deneyin.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Kullanıcıyı Uyar', ban_response.message ? ban_response.message : 'Bu kullanıcı, gönderisinden dolayı başarılı bir şekilde uyarıldı.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Kullanıcıyı Uyar', ban_response.message ? ban_response.message : 'Kullanıcıya uyarı gönderirken bir hatayla karşılaştık, lütfen daha sonra tekrar deneyin.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Kullanıcıyı Yasakla', 'Kullanıcıları yasaklama iznine sahip olmayabilirsiniz. Lütfen hesabınızın izinlerini doğrulayın ya da daha sonra tekrar deneyin.' );
		}
		else {
			ShowAlertDialog( 'Kullanıcıyı Uyar', 'Kullanıcıya uyarı gönderirken bir hatayla karşılaştık, lütfen daha sonra tekrar deneyin.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Lütfen şu kullanıcıya özel olarak göndermek istediğiniz mesajı girin:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Bu yazı özel mesajınızda otomatik olarak atıf yapılacaktır.</div>';

	var $modal = ShowConfirmDialog( 'Özel Mesaj', $strDialogHTML, 'Özel Mesaj Gönder' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Özel Mesaj', response.message );
				}
				else
				{
					ShowAlertDialog( 'Hata', 'Bu özel mesajı gönderirken bir hata oluştu. Lütfen tekrar deneyin.' + '(' + response.success + ')' );
				}
			});
	} );
