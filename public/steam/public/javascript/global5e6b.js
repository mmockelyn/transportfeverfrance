	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Innehållet kanske inte är lämpligt för alla åldrar', data['html'], 'Visa innehåll' );
				...
				{
					ShowAlertDialog('Innehållet kanske inte är lämpligt för alla åldrar', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Visa varning om vuxet innehåll' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Ta endast bort censurering för detta innehåll' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Censurera detta innehåll' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Innehållet kan vara olämpligt för vissa målgrupper'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Markera som spam', 'Detta utdelar en gemenskapsavstängning till skaparen. Vi lägger även till den här kommentaren i vår inlärningsdata så att vi automatiskt kan flagga på den här typen av kommentarer och radera alla kommentarer från skaparen i framtiden. Är du säker på att du vill fortsätta?', 'Markera som spam' );
