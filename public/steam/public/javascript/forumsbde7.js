...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Ha a raktárad láthatósága Nyilvános, a közösség tagjai tudnak csereajánlatokat küldeni neked ebből a témából.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>A raktárad láthatósági beállítása Nyilvános</b>. Ez azt jelenti, hogy a közösség tagjai küldhetnek neked csereajánlatokat ebből a témából. Ha a raktárad láthatósági beállítása Nyilvános helyett bármi másra van állítva, nem tudsz közösségi csereajánlatokat kapni.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>A raktárad láthatósági beállítása <u>nem</u> Nyilvános</b>. Ez azt jelenti, hogy a közösség tagjai nem küldhetnek neked csereajánlatokat ebből a témából. Ha raktárad láthatósági beállítása Nyilvános lenne, a közösség tagjai tudnának csereajánlatokat küldeni neked ebből a témából.';
			...
			strMessage += 'Mikor nem kérsz több csereajánlatot, vagy végrehajtottad a cserét, bezárhatod ezt a témát, és letilthatod az innen érkező ajánlatokat.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Hiba történt az új téma indítása során: ' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Feliratkozás a fórumra', 'Hiba történt a fórumra való feliratkozás közben.', 'Kérjük, próbáld újra később.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Leiratkozás a fórumról', 'Hiba történt a fórumról való leiratkozás közben.', 'Kérjük, próbáld újra később.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Téma törlése ',
				'Biztosan törölni szeretnéd ezt a témát? ' +
				'Ezt a műveletet csak egy moderátor vonhatja vissza.',
				'Téma törlése'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Végleges törlés ',
		'Biztosan végleg törlöd ezt a témát? A művelet nem visszavonható.',
		'Végleges törlés'
	).done( function() {
...
{
	ShowConfirmDialog('Tiltandó tartalom jelölés törlése ',
		'Biztosan törölni akarod a tiltandó tartalom megjelölést erről a témáról? Ezt nem lehet visszacsinálni.',
		'Tiltandó tartalom jelölés törlése'
	).done( function() {
...
{
	ShowConfirmDialog('Hozzászólók kitiltása ',
	'Biztosan ki akarod tiltani az ebben a témában 5 alkalomnál többször hozzászóló felhasználókat? Ezt nehéz lesz visszacsinálni.',
	'Hozzászólók kitiltása'
).done( function() {
...
{
	ShowConfirmDialog('Spamnek jelölés ',
		'Ez törli a kiválasztott témákat, és kitiltja a téma szerzőit a fórumból. Biztosan ezt akarod tenni?',
		'Spam törlése'
	).done( function() {
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						'Témák egyesítése',
						'A kiválasztott témák egyesítésre kerültek egyetlen témába.',
						'Egyesített téma megnyitása',
...
{
	ShowConfirmDialog( 'Téma lezárása', 'A téma „zártként” jelölése meggátolja további válaszok vagy csereajánlatok küldését. Lezárása után nem tudsz majd további műveleteket végezni rajta.', 'Téma lezárása')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Jelentés', 'Köszönjük a jelentésed!', 'A jelentésed el lett küldve a moderátoroknak felülvizsgálatra. Értesíteni fogunk, ha intézkedésre kerül sor.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Nem sikerült a téma módosítása. Hibaüzenet:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Nem sikerült módosítani a témát. Kérjük, próbáld újra később.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Feliratkozás a témára', 'Feliratkoztál erre a témára.', 'Hozzászólás-értesítést fogsz kapni, valahányszor valaki válaszol a témában.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Feliratkozás a témára', 'Sajnáljuk, nem sikerült a feliratkozás erre a témára. Próbáld újra később.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Leiratkozás a témáról', 'Leiratkoztál erről a témáról.', 'Többé nem kapsz hozzászólás-értesítéseket erről a témáról.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Leiratkozás a témáról', 'Sajnáljuk, nem sikerült a leiratkozás erről a témáról. Próbáld újra később.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Az e témához tartozó összes jelentés megoldott jelölést kapott.<br>A megoldást igényelt jelentések száma: ' + cReportsResolved;
				else
					strMessage = 'Az e témához tartozó összes jelentés már korábban megoldott jelölést kapott.';

				ShowForumSuccessDialogWithDetailTitle( 'Jelentett hozzászólások kezelése', 'Jelentések megoldva.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Hozzászólás-előzmények', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Biztosan törölni szeretnéd ezt a bejegyzést?', 'Törlés' )
...
var g_rgRedirectTimeOptions = [
	{value: 0, label: 'Nincs átirányítás' },
	{value: 1, label: '1 óra' },
	{value: 24, label: '1 nap' },
	{value: 168, label: '1 hét' },
	{value: 720, label: '1 hónap' },
	{value: -1, label: 'végleges' }
];
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'A témák összevonása egy vagy több téma összes hozzászólását áthelyezi egy másik témába. Ha a céltémában már vannak hozzászólások, az új hozzászólások a közzétételi dátum alapján kerülnek berendezésre közéjük. Ezt nem lehet visszavonni.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Válaszd ki azt a nemrég megnézett témát, amibe a kiválasztott témákat bele akarod vonni:' : 'Válaszd ki azt a nemrég megnézett témát, amibe ezt bele akarod vonni:';
	...
	var strMoreInstructions = 'Ha a téma, amivel szeretnél egyesíteni nincs itt felsorolva, <%1$s>megkeresheted a kívánt témát<%2$s> egy új ablakban. Ez a lista automatikusan frissítésre kerül, amikor visszatérsz.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Átirányítás ennyi ideig: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Kiválasztott témák egyesítése a legrégebbibe' );
	...
	var Modal = ShowConfirmDialog( 'Témák egyesítése', $DialogContent, 'Témák egyesítése' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Helyreállítás', 'A kiválasztott témák visszaállításra kerültek.' );
			else
				ShowAlertDialog( 'Törlés', 'A kiválasztott témák törlésre kerültek.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Feloldás', 'A kiválasztott témák feloldásra kerültek.' );
			else
				ShowAlertDialog( 'Zárolás', 'A kiválasztott témák zárolásra kerültek.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Áthelyezés...', 'A kiválasztott témák áthelyezésre kerültek az új fórumra.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Témák egyesítése',
					'A kiválasztott témák egyesítésre kerültek egyetlen témába.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Végleges törlés ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Végleges törlés', 'A kijelölt témák végleg törlésre kerültek.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Spamnek jelölés', 'A kiválasztott témák törlődtek, a témanyitók tiltásra kerültek.' );
		});
});
}
...
{
	ShowConfirmDialog('Hozzászólók kitiltása ',
	'Közösségi kitiltást ad a kiválasztott témákban egy adott alkalomnál többször hozzászóló felhasználóknak (vagy eltérítettnek jelöli őket). Biztos vagy benne, hogy ezt akarod tenni?',
	'Hozzászólók kitiltása'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Hozzászólók kitiltása', 'A kiválasztott témákban a küszöbnél többször hozzászóló felhasználók ki lettek tiltva.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Hiba', 'Hiba történt kérésed egészének vagy egy részének feldolgozása közben. Ellenőrizd, hogy a várt eredményt kaptad-e.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Felhasználó figyelmeztetése', $WaitElem, 'Felhasználó figyelmeztetése' );
	} else {
		var Modal = ShowConfirmDialog( 'Kitiltás', $WaitElem, 'Kitiltás' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Kitiltás', 'Legalább egy lehetőség kiválasztása kötelező. A felhasználó nem lett kitiltva.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Kitiltás', ban_response.message ? ban_response.message : 'A felhasználó hozzászólási és szerkesztési jogai megvonásra kerültek.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Kitiltás', ban_response.message ? ban_response.message : 'Lehet, hogy nincs jogod felhasználókat kitiltani. Ellenőrizd fiókod jogosultságait, vagy próbáld újra később.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Felhasználó figyelmeztetése', ban_response.message ? ban_response.message : 'A felhasználó figyelmeztetése a hozzászólása miatt sikeres volt.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Felhasználó figyelmeztetése', ban_response.message ? ban_response.message : 'Hibába ütköztünk a felhasználó figyelmeztetésekor, próbáld újra később.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Kitiltás', 'Lehet, hogy nincs jogod felhasználókat kitiltani. Ellenőrizd fiókod jogosultságait, vagy próbáld újra később.' );
		}
		else {
			ShowAlertDialog( 'Felhasználó figyelmeztetése', 'Hibába ütköztünk a felhasználó figyelmeztetésekor, próbáld újra később.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Írd ide az üzenetet, amit privát módon szeretnél ennek a felhasználónak küldeni:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Ez a hozzászólás automatikusan hivatkozva lesz a privát üzenetedben.</div>';

	var $modal = ShowConfirmDialog( 'Privát üzenet', $strDialogHTML, 'Privát üzenet küldése' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Privát üzenet', response.message );
				}
				else
				{
					ShowAlertDialog( 'Hiba', 'Hiba történt a privát üzenet elküldése közben. Próbáld újra.' + '(' + response.success + ')' );
				}
			});
	} );
