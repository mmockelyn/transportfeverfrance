...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Última atualização'} ) );
...
function OnIsCompanyChange()
{
	...
	else
	{
		...
		$('firstnamelabel').innerHTML = 'Nome (igual aos registros bancários)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Número de roteamento (Routing Number)';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Número de Trânsito Canadense (Canadian Transit Number)';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Informe um nome de empresa.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Informe um primeiro nome válido.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Informe um sobrenome válido.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Informe um endereço válido.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Informe uma cidade válida.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Informe um código zip ou postal (CEP) válido.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Informe o seu nº de telefone com o código de área (DDD).<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Salvos!', response.redirect_url.length != 0 ? 'Os dados de contato e endereço foram salvos. Já que alterou o seu país, você será redirecionado a um serviço externo para atualizar os seus dados tributários.' : 'Os dados de contato e endereço foram salvos. Preencha os seus dados bancários e tributários caso ainda não o tenha feito.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Erro', 'Ocorreu um erro desconhecido ao tentar salvar os dados de contato.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Informe um endereço de e-mail válido.<br/>';
		}
		...
		{
			errorString += 'Informe um nome de banco válido.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Informe um nome de titular de conta bancária válido (deve coincidir com os registros do banco).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Informe um endereço válido para o seu banco.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Informe uma cidade válida para o seu banco.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Informe um código zip ou postal (CEP) válido para o seu banco.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Informe o número da sua conta no banco.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Informe o número de roteamento (Routing Number) do seu banco (9 dígitos).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Informe o Número de Trânsito Canadense (Canadian Transit Number ) (9 dígitos com um 0 na frente).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Informe o IBAN do seu banco.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Informe o código SWIFT do seu banco.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Gerenciar colaboradores', 'Carregando lista de amigos' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Gerenciar colaboradores', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Cancelar' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Erro', 'Não foi possível encontrar o perfil especificado.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Tem certeza de que deseja finalizar a lista de colaboradores? Não será possível adicioná-los ou removê-los após este passo.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Aguarde', 'Carregando opções...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Selecionar os prestadores de serviço';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Erro', 'Os percentuais de receita dos prestadores de serviço devem somar 100%.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Erro', 'Não é possível escolher uma organização de fora que também é um colaborador normal.' );
				break;
			default:
				ShowAlertDialog( 'Erro', 'Ocorreu um erro ao processar a sua solicitação:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
