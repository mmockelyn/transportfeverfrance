...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Jeśli twój ekwipunek jest publiczny, to członkowie Społeczności Steam będą mogli przy pomocy tego wątku wysyłać ci oferty wymiany.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>Prywatność twojego ekwipunku jest ustawiona jako publiczna</b>. Oznacza to, że przy pomocy tego tematu użytkownicy Społeczności Steam będą w stanie wysyłać ci propozycje wymiany. Jeśli prywatność twojego ekwipunku zostanie ustawiona na inną niż publiczna, to nie będziesz otrzymywać ofert wymiany od członków Społeczności Steam.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>Prywatność twojego ekwipunku <u>nie jest</u> ustawiona jako publiczna</b>. Oznacza to, że użytkownicy Społeczności Steam nie będą w stanie wysyłać ci propozycji wymiany przy pomocy tego tematu. Gdyby prywatność twojego ekwipunku byłaby ustawiona jako publiczna, to użytkownicy Społeczności Steam mogliby przy pomocy tego tematu wysyłać ci propozycje wymiany.';
			...
			strMessage += 'Kiedy uznasz, że nie chcesz już otrzymywać więcej ofert lub wymiana zostanie dokonana, możesz zamknąć ten wątek i uniemożliwić użytkownikom składanie dalszych ofert.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Podczas tworzenia nowego wątku wystąpił błąd: ' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Subskrybuj forum', 'Wystąpił błąd podczas zapisywania się do tego forum.', 'Spróbuj ponownie później.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Przestań obserwować to forum', 'Wystąpił błąd podczas wypisywania się z tego forum.', 'Spróbuj ponownie później.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Usuń wątek ',
				'Czy na pewno chcesz usunąć ten wątek? ' +
				'Ta czynność może zostać cofnięta tylko przez moderatora.',
				'Usuń wątek'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Usuń na zawsze ',
		'Czy na pewno chcesz całkowicie usunąć ten wątek? Tej czynności nie da się cofnąć.',
		'Usuń na zawsze'
	).done( function() {
...
{
	ShowConfirmDialog('Wyczyść oznaczenie niedozwolonej zawartości ',
		'Czy na pewno chcesz wyczyścić oznaczenie niedozwolonej zawartości dla tego wątku? Tego działania nie można cofnąć.',
		'Wyczyść oznaczenie niedozwolonej zawartości'
	).done( function() {
...
{
	ShowConfirmDialog('Zablokuj komentujących ',
	'Czy na pewno chcesz zablokować użytkowników, którzy skomentowali ten wątek więcej niż 5 razy? To działanie będzie trudne do wycofania.',
...
{
	ShowConfirmDialog('Oznacz spam ',
		'To działanie usunie zaznaczone wątki i zablokuje autorów z forum. Czy na pewno chcesz to zrobić?',
		'Usuń spam'
	).done( function() {
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						'Połącz wątki',
						'Zaznaczone wątki zostały połączone w jeden.',
						'Przejdź do połączonego wątku',
...
{
	ShowConfirmDialog( 'Zamknij ten wątek', 'Oznaczenie tego wątku jako zamknięty sprawi, że nie będą się w nim mogły pojawiać nowe odpowiedzi czy propozycje wymiany. Jeśli zdecydujesz się go zamknąć, to w przyszłości nie będziesz w stanie nic z nim zrobić.', 'Zamknij ten wątek')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Zgłoś', 'Dziękujemy za twoje zgłoszenie', 'Twoje zgłoszenie zostało wysłane do moderatorów. Damy ci znać, jeśli podejmiemy jakieś działania.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Nie udało się zmodyfikować wątku. Komunikat o błędzie:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Nie udało się zmodyfikować wątku. Spróbuj ponownie później.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Obserwuj tę dyskusję', 'Obserwujesz tę dyskusję.', 'Otrzymasz powiadomienie o komentarzu za każdym razem, kiedy ktoś odpowie w tej dyskusji.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Obserwuj tę dyskusję', 'Przepraszamy, nie możesz obserwować tej dyskusji. Spróbuj ponownie później.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Przestań obserwować tę dyskusję', 'Zaprzestano obserwowania tej dyskusji.', 'Nie będziesz już otrzymywać powiadomień o tej dyskusji.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Przestań obserwować tę dyskusję', 'Przepraszamy, nie można zaprzestać obserwowania tego tematu. Spróbuj ponownie później.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Wszystkie zgłoszenia powiązane z tym wątkiem zostały oznaczone jako rozpatrzone.<br>Liczba zgłoszeń, które wymagały rozpatrzenia: ' + cReportsResolved;
				else
					strMessage = 'Wszystkie zgłoszenia powiązane z tym wątkiem zostały już oznaczone jako rozpatrzone.';

				ShowForumSuccessDialogWithDetailTitle( 'Rozwiąż zgłoszone posty', 'Zgłoszenia rozpatrzone.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Historia wątku', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Czy na pewno chcesz usunąć ten post?', 'Usuń' )
...
var g_rgRedirectTimeOptions = [
	{value: 0, label: 'Brak odnośnika' },
	{value: 1, label: '1 godzina' },
	{value: 24, label: '1 dzień' },
	{value: 168, label: 'Tydzień' },
	{value: 720, label: 'Miesiąc' },
	{value: -1, label: 'Na zawsze' }
];
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'Połączenie wątków spowoduje przeniesienie wszystkich postów z jednego lub więcej tematów do innego wątku. Jeśli temat docelowy zawiera posty, to przeniesione posty zostaną w nim ułożone według czasu. Tej czynności nie da się cofnąć.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Wybierz ostatnio wyświetlany wątek, z którym chcesz połączyć zaznaczone wątki:' : 'Wybierz ostatnio wyświetlany wątek, z którym chcesz połączyć zaznaczony wątek:';
	...
	var strMoreInstructions = 'Jeśli wątek, z którym chcesz połączyć inne, nie jest tutaj wyświetlony, możesz <%1$s>wyszukać go<%2$s> w nowym oknie. Lista zostanie automatycznie zaktualizowana po zamknięciu okna.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Zostaw odnośnik: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Połącz zaznaczone wątki w najstarszy z nich' );
	...
	var Modal = ShowConfirmDialog( 'Połącz wątki', $DialogContent, 'Połącz wątki' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Przywróć', 'Zaznaczone wątki zostały przywrócone.' );
			else
				ShowAlertDialog( 'Usuń', 'Zaznaczone wątki zostały usunięte.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Otwórz', 'Zaznaczone wątki zostały otwarte.' );
			else
				ShowAlertDialog( 'Zamknij', 'Zaznaczone wątki zostały zamknięte.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Przenieś...', 'Zaznaczone wątki zostały przeniesione do nowego forum.' );
		});
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Usuń na zawsze ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Usuń na zawsze', 'Zaznaczone wątki zostały trwale usunięte.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Oznacz spam', 'Zaznaczone wątki zostały usunięte, a ich autorzy zablokowani.' );
		});
});
}
...
{
	ShowConfirmDialog('Zablokuj komentujących ',
	'Nałoży to blokadę społeczności na użytkowników (lub oznaczy ich konta jako przejęte), którzy napisali komentarz w wybranych wątkach więcej niż daną liczbę razy. Czy na pewno chcesz to zrobić?',
	'Zablokuj komentujących'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Zablokuj komentujących', 'Użytkownicy, którzy przekroczyli limit dopuszczalnych komentarzy w wybranych wątkach, zostali zablokowani.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Błąd', 'Wystąpił błąd podczas przetwarzania wszystkich lub części twoich żądań. Zweryfikuj czy oczekiwany rezultat został osiągnięty.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Wystaw ostrzeżenie', $WaitElem, 'Wystaw ostrzeżenie' );
	} else {
		var Modal = ShowConfirmDialog( 'Zablokuj użytkownika', $WaitElem, 'Zablokuj użytkownika' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Zablokuj użytkownika', 'Co najmniej jedna opcja musi zostać wybrana. Użytkownik nie został zbanowany.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Zablokuj użytkownika', ban_response.message ? ban_response.message : 'Możliwość dodawania i edytowania postów przez użytkownika została zablokowana.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Zablokuj użytkownika', ban_response.message ? ban_response.message : 'Wygląda na to, że nie posiadasz uprawnień, by blokować użytkowników. Sprawdź uprawnienia swojego konta, a następnie spróbuj ponownie.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Wystaw ostrzeżenie', ban_response.message ? ban_response.message : 'Użytkownik został pomyślnie ostrzeżony za ten post.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Wystaw ostrzeżenie', ban_response.message ? ban_response.message : 'Wystąpił problem podczas wystawiania ostrzeżenia. Spróbuj ponownie później.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Zablokuj użytkownika', 'Wygląda na to, że nie posiadasz uprawnień, by blokować użytkowników. Sprawdź uprawnienia swojego konta, a następnie spróbuj ponownie.' );
		}
		else {
			ShowAlertDialog( 'Wystaw ostrzeżenie', 'Wystąpił problem podczas wystawiania ostrzeżenia. Spróbuj ponownie później.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Wprowadź treść prywatnej wiadomości, którą chcesz wysłać do tego użytkownika:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Odwołanie do tego postu zostanie automatycznie dodane do twojej prywatnej wiadomości.</div>';

	var $modal = ShowConfirmDialog( 'Prywatna wiadomość', $strDialogHTML, 'Wyślij prywatną wiadomość' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Prywatna wiadomość', response.message );
				}
				else
				{
					ShowAlertDialog( 'Błąd', 'Wystąpił błąd podczas wysyłania prywatnej wiadomości. Spróbuj ponownie.' + '(' + response.success + ')' );
				}
			});
	} );
