...
{
	ShowConfirmDialog('Изчистване на флага относно съдържание за забрана ',
		'Сигурни ли сте, че искате да изчистите флага относно съдържанието за забрана върху тази тема? Това не може да бъде отменено.',
		'Изчистване на флага относно съдържание за забрана'
	).done( function() {
...
{
	ShowConfirmDialog('Маркиране на спам ',
...
CForumTopic = Class.create( {
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Неуспешно променяне на темата. Моля, опитайте отново по-късно.' );
		}
	},
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Премахване на абонамента', 'Отписани сте от абонамента Ви към тази дискусия.', 'Вече няма да получавате известия за коментари в тази дискусия.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Премахване на абонамента', 'Съжаляваме, но нямахме възможност да отпишем абонамента Ви за тази дискусия. Моля, опитайте отново по-късно.');
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Маркиране на спам', 'Избраните теми бяха изтрити, а на създателите е наложена забрана.' );
		});
});
}
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
			...
			Modal.done( function() {
				...
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Забрана на потребител', ban_response.message ? ban_response.message : 'Може би нямате привилегии да наложите забрани на потребители. Моля, проверете привилегиите на акаунта си и опитайте отново по-късно.' );
					});
				}
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Забрана на потребител', 'Може би нямате привилегии да наложите забрани на потребители. Моля, проверете привилегиите на акаунта си и опитайте отново по-късно.' );
		}
		else {
