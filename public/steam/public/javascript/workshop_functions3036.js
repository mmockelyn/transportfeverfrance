...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Publiseringsdato' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Mellom' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Og' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Sist oppdatert'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Mellom' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Og' } ) );
	...
	var dialog = ShowConfirmDialog( 'Filtrer etter dato', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Firmanavn';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Fornavn (lik det registrert på bankkontoen)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Rutingnummer';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Kanadisk overføringsnummer';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Vennligst skriv inn et selskapsnavn.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Vennligst skriv inn et gyldig fornavn.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Vennligst skriv inn et gyldig etternavn.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Vennligst skriv inn en gyldig adresse.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vennligst skriv inn en gyldig by.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Vennligst skriv inn et gyldig postnummer.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Vennligst skriv inn ditt telefonnummer, inkludert retningsnummer.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Lagret!', response.redirect_url.length != 0 ? 'Din kontakt- og adresseinformasjon er lagret. Siden du har oppdatert landet ditt, vil du nå bli omdirigert til en ekstern tjeneste for å oppdatere skatteinformasjonen din.' : 'Din kontakt- og adresseinformasjon er lagret. Om du ikke allerede har det, vennligst fyll inn din skatte- og bankinformasjon også.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Feil', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Feil', 'En ukjent feil oppsto under forsøket på å lagre din kontaktinformasjon.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Vennligst skriv inn en gyldig e-postadresse.<br/>';
		}
		...
		{
			errorString += 'Skriv inn et gyldig banknavn.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vennligst skriv inn et gyldig navn på bankkonto-eier (må stemme overens med bankens informasjon).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vennligst skriv en gyldig adresse for banken din.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vennligst skriv inn en gyldig by for banken din.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Skriv inn et gyldig postnummer for banken din.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Vennligst skriv inn ditt bankkontonummer.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Vennligst skriv inn din banks rutingnummer (9 siffer)<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Vennligst skriv inn det kanadiske overføringsnummeret (9 siffer som starter med 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Vennligst skriv inn IBAN til banken din.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Vennligst skriv inn banken din sin SWIFT-kode.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Behandle bidragsytere', 'Laster venneliste' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Behandle bidragsytere', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Avbryt' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Feil', 'Finner ikke den angitte profilen.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Er du sikker på at du vil ferdigstille bidragsyterne? Du vil ikke kunne legge til eller fjerne de etter dette steget.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Vent litt', 'Laster alternativer …' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Velg tjenesteleverandører';
		var strSaveChanges = 'Fortsett';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Feil', 'Tjenesteleverandørenes prosentsatser for inntekt må kunne summeres til å bli 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Feil', 'Du kan ikke velge en tredjepartsorganisasjon som også er en normal bidragsyter til din gjenstand.' );
				break;
			default:
				ShowAlertDialog( 'Feil', 'Det oppstod en feil under behandling av forespørselen:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
