...
var CForum = Class.create( {
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Подписаться на форум', 'При подписке на форум произошла ошибка.', 'Пожалуйста, попробуйте позднее.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Отписаться от форума', 'При отписке от форума произошла ошибка.', 'Пожалуйста, попробуйте позднее.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
{
	ShowConfirmDialog('Снять пометку о блокируемом контенте ',
		'Вы уверены, что хотите снять с этой темы пометку о блокируемом контенте? Это действие нельзя отменить.',
		'Снять пометку о блокируемом контенте'
	).done( function() {
...
{
	ShowConfirmDialog('Заблокировать комментаторов ',
	'Вы уверены, что хотите заблокировать пользователей, которые оставили больше 5 сообщений в этой теме? Это действие нельзя отменить.',
...
{
	ShowConfirmDialog('Отметить как спам ',
		'Выбранные темы будут удалены, а их авторы — заблокированы. Вы уверены, что хотите сделать это?',
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						'Объединить темы',
						'Выделенные темы объединены в одну.',
...
{
	ShowConfirmDialog( 'Закрыть тему', 'Пометив эту тему как закрытую, вы запретите писать или предлагать сделки в ней. После ее закрытия вы не сможете осуществлять в ней какие-либо действия.', 'Закрыть тему')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Пожаловаться', 'Спасибо за вашу бдительность', 'Жалоба отправлена модераторам. Мы сообщим вам, если по итогам проверки будут приняты меры.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Не удалось изменить тему. Ошибка:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Не удалось отредактировать тему. Пожалуйста, повторите попытку позже.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Подписаться на обсуждение', 'Вы подписались на это обсуждение.', 'Вы получите уведомление, когда кто-нибудь продолжит обсуждение.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Подписаться на обсуждение', 'Не удалось подписаться на это обсуждение. Повторите попытку позже.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Отписаться от обсуждения', 'Вы отписались от этого обсуждения.', 'Вы больше не будете получать уведомления о комментариях в этом обсуждении.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Отписаться от обсуждения', 'Не удалось отписаться от этого обсуждения. Повторите попытку позже.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Все жалобы в этой теме помечены как рассмотренные.<br>Общее число рассмотренных жалоб: ' + cReportsResolved;
				else
					strMessage = 'Все жалобы в этой теме уже помечены как рассмотренные.';

				ShowForumSuccessDialogWithDetailTitle( 'Рассмотреть жалобы на сообщения', 'Жалобы рассмотрены.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'История правок', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Вы уверены, что хотите удалить это сообщение?', 'Удалить' )
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	strMessage += rgForumTopics.length > 1 ? 'Выберите недавно просмотренную тему, с которой нужно объединить отмеченные темы:' : 'Выберите недавно просмотренную тему, с которой нужно объединить текущую:';
	...
	var strMoreInstructions = 'Если тема, с которой вы хотите объединить, отсутствует в списке, вы можете <%1$s>найти<%2$s> её в новом окне. Список выше будет автоматически обновлён.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Срок перенаправления: ', $Select );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Восстановить', 'Выделенные темы восстановлены.' );
			else
				ShowAlertDialog( 'Удалить', 'Выделенные темы удалены.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Открыть', 'Выделенные темы открыты.' );
			else
				ShowAlertDialog( 'Закрыть', 'Выделенные темы закрыты.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Переместить…', 'Выделенные темы перемещены в новый форум.' );
		});
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Удалить навсегда ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Удалить навсегда', 'Выбранные темы удалены навсегда.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Отметить как спам', 'Выбранные темы удалены, а их авторы заблокированы.' );
		});
});
}
...
{
	ShowConfirmDialog('Заблокировать комментаторов ',
	'Вы заблокируете в сообществе (или пометите как скомпрометированные) аккаунты, которые превысили порог сообщений в выбранных темах. Вы уверены, что хотите сделать это?',
	'Заблокировать комментаторов'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Заблокировать комментаторов', 'Все пользователи, превысившие порог сообщений в выбранных темах, заблокированы.' );
		});
});
}
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Выдать предупреждение', $WaitElem, 'Выдать предупреждение' );
	} else {
		var Modal = ShowConfirmDialog( 'Заблокировать пользователя', $WaitElem, 'Заблокировать пользователя' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Заблокировать пользователя', 'Должен быть выбран по крайней мере один пункт. Пользователь не был заблокирован.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Заблокировать пользователя', ban_response.message ? ban_response.message : 'Этот пользователь больше не может писать и редактировать сообщения.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Заблокировать пользователя', ban_response.message ? ban_response.message : 'Возможно, у вас нет полномочий для блокировки пользователей. Убедитесь, что обладаете такими полномочиями, или повторите попытку позже.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Выдать предупреждение', ban_response.message ? ban_response.message : 'Предупреждение выдано.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Выдать предупреждение', ban_response.message ? ban_response.message : 'Не удалось выдать предупреждение. Пожалуйста, повторите попытку позже.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Заблокировать пользователя', 'Возможно, у вас нет полномочий для блокировки пользователей. Убедитесь, что обладаете такими полномочиями, или повторите попытку позже.' );
		}
		else {
			ShowAlertDialog( 'Выдать предупреждение', 'Не удалось выдать предупреждение. Пожалуйста, повторите попытку позже.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Сообщение пользователю:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Этот текст будет автоматически указан в вашем сообщении.</div>';

	var $modal = ShowConfirmDialog( 'Личное сообщение', $strDialogHTML, 'Отправить' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Личное сообщение', response.message );
				}
				else
				{
					ShowAlertDialog( 'Ошибка', 'Не удалось отправить сообщение. Пожалуйста, попробуйте ещё раз.' + '(' + response.success + ')' );
				}
			});
	} );
