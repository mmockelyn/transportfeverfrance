	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Indholdet er muligvis ikke egnet til alle aldre', data['html'], 'Vis indhold' );
				...
				{
					ShowAlertDialog('Indholdet er muligvis ikke egnet til alle aldre', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Vis advarsel om voksenindhold' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Fjern tilsløring for dette indhold' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Tilslør dette indhold' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Indholdet er muligvis ikke egnet for alle'} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Abonner på kommentarer', 'Der opstod et problem med opdatering af dit abonnement. Prøv venligst igen senere.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Abonner på kommentarer', 'Der opstod et problem med opdatering af dit abonnement. Prøv venligst igen senere.');});
		});
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Markér som spam', 'Dette giver en fællesskabsudelukkelse til forfatteren. Tilføj denne kommentar som "læringsdata", så vi kan markere denne slags kommentarer automatisk og rydde alle forfatterens kommentarer. Er du sikker på, at du ønsker at gøre dette?', 'Markér som spam' );
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Abonner på kommentarer', 'Du modtager en notifikation, når nogen skriver i disse kommentarer.');
			}, function() {
				ShowAlertDialog('Abonner på kommentarer', 'Der opstod et problem med opdatering af dit abonnement. Prøv venligst igen senere.');
			});
		});
