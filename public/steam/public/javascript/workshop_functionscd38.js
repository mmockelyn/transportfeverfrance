...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Date de publication' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'entre' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'et' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Date de la dernière mise à jour'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'entre' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'et' } ) );
	...
	var dialog = ShowConfirmDialog( 'Trier par date', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Nom de société';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Prénom (comme écrit sur les relevés bancaires)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Numéro de routage';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Numéro de transit Canadien';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Veuillez entrer le nom d\'une entreprise.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Veuillez entrer un prénom valide.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Veuillez entrer un nom de famille valide.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Veuillez entrer une adresse postale valide.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Veuillez entrer une ville valide.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Veuillez entrer un code postal ou zip valide.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Veuillez entrer votre numéro de téléphone, ainsi que l\'indicatif.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Enregistrées !', response.redirect_url.length != 0 ? 'Vos informations de contact et votre adresse ont été enregistrées. Suite à la mise à jour de votre pays, vous allez être redirigé(e) sur un service externe afin de mettre à jour vos renseignements fiscaux.' : 'Vos informations de contact et l\'adresse ont été enregistrées. Si cela n\'est pas déjà fait, veuillez remplir vos informations bancaires et fiscales.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Erreur', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Erreur', 'Une erreur est survenue lors de la sauvegarde de vos informations de contact.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Veuillez entrer une adresse e-mail valide.<br/>';
		}
		...
		{
			errorString += 'Veuillez entrer un nom de banque valide.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Veuillez entrer un nom de titulaire de compte bancaire valide (doit correspondre au nom apparaissant sur les relevés bancaires).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Veuillez entrer une adresse de banque valide.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Veuillez entrer une ville valide pour votre banque.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Veuillez entrer un code postal ou zip valide pour votre banque.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Veuillez entrer votre numéro de compte en banque.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Veuillez entrer le numéro de routage de votre banque (9 chiffres).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Veuillez entrer le numéro de transit Canadien (9 numéros commencent par un 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Veuillez entrer l\'IBAN de votre banque.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Veuillez entrer le code SWIFT de votre banque.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Gérer les contributeurs', 'Chargement de la liste de contacts' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Gérer les contributeurs', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Annuler' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Erreur', 'Profil spécifié introuvable.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Voulez-vous vraiment finaliser la liste des contributeurs ? Vous ne pourrez plus en ajouter ou en enlever après cette étape.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Veuillez patienter', 'Chargement des options...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Sélectionnez des prestataires de services';
		var strSaveChanges = 'Continuer';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Erreur', 'Le total des pourcentages de recettes des prestataires de services doit être égal à 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Erreur', 'Vous ne pouvez pas choisir une organisation tierce qui est également un contributeur normal sur votre objet.' );
				break;
			default:
				ShowAlertDialog( 'Erreur', 'Une erreur est survenue lors du traitement de votre requête :' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
