...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Opslået' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'mellem' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Sidst opdateret'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'mellem' } ) );
	...
	var dialog = ShowConfirmDialog( 'Filtrer efter dato', content );
...
function OnIsCompanyChange()
{
	...
	else
	{
		...
		$('firstnamelabel').innerHTML = 'Fornavn (som skrevet i bankjournaler)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Registreringsnummer';
		}
		else
		{
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Indtast venligst et firmanavn.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Indtast venligst et gyldigt fornavn.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Indtast venligst et gyldigt efternavn.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Indtast venligst en gyldig adresse.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Indtast venligst en gyldig by.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Indtast venligst et gyldig postnummer.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Indtast venligst dit telefonnummer inklusiv områdenummer.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Gemt!', response.redirect_url.length != 0 ? 'Dine kontakt- og adresseoplysninger er blevet gemt. Idet du har opdateret dit land, vil du nu blive videresendt til en ekstern tjeneste til at opdatere dine skatteoplysninger.' : 'Dine kontakt- og adresseoplysninger er blevet gemt. Hvis du ikke allerede har gjort det så udfyld venligst dine bankkonto- og skatteoplysninger.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Fejl', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Fejl', 'Der opstod en ukendt fejl i forsøget på at gemme dine kontaktoplysninger.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Indtast en gyldig e-mailadresse.<br/>';
		}
		...
		{
			errorString += 'Indtast venligst et gyldig banknavn.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Indtast venligst et gyldigt navn på bankkontoindehaveren (skal matche bankjournaler).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Indtast venligst en gyldig adresse til din bank.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Indtast venligst en gyldig by til din bank.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Indtast venligst et gyldigt postnummer til din bank.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Indtast venligst dit bankkontonummer.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Indtast venligst din banks registreringsnummer (9 cifre).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Indtast venligst nummeret "Canadian Transit Number" (9 cifre med et indledende 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Indtast venligst din banks IBAN.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Indtast venligst din banks SWIFT-kode.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Administrer bidragere', 'Indlæser venneliste' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Administrer bidragere', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Annuller' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Fejl', 'Den angivne profil blev ikke fundet.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Er du sikker på, at du vil færdiggøre bidragyder-listen? Du vil ikke kunne tilføje eller fjerne dem efter dette trin.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Vent venligst', 'Indlæser indstillinger...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Vælg tjenesteudbydere';
		var strSaveChanges = 'Fortsæt';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Fejl', 'Tjenesteudbyder-indtægtsprocenterne skal sammenlagt give 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Fejl', 'Du kan ikke vælge en tredjepartsorganisation, som også er en normal bidragyder til dit emne.' );
				break;
			default:
				ShowAlertDialog( 'Fejl', 'Der skete en fejl ved behandling af din forespørgsel:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
