	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Questo contenuto potrebbe non essere adatto a tutte le età', data['html'], 'Visualizza contenuto' );
				...
				{
					ShowAlertDialog('Questo contenuto potrebbe non essere adatto a tutte le età', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Mostra un avviso per i contenuti non adatti ai minori' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Rimuovi la censura soltanto da questo contenuto' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Censura questo contenuto' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Il contenuto potrebbe non essere adatto a tutte le età'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Segna come spam', 'L\'autore sarà bandito dalla Comunità, il commento sarà aggiunto ai dati che usiamo per il miglioramento della funzione per contrassegnare automaticamente questo tipo di commenti e tutti i commenti dell\'autore saranno rimossi. Confermi?', 'Segna come spam' );
