	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('O conteúdo pode não ser apropriado para todas as idades', data['html'], 'Exibir conteúdo' );
				...
				{
					ShowAlertDialog('O conteúdo pode não ser apropriado para todas as idades', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Ver aviso de conteúdo adulto' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Remover borrão apenas para este conteúdo' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Borrar conteúdo' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'O conteúdo pode não ser apropriado para todas as idades'} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Inscrever-se no tópico', 'Houve um problema ao atualizar a sua inscrição. Tente novamente mais tarde.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Inscrever-se no tópico', 'Houve um problema ao atualizar a sua inscrição. Tente novamente mais tarde.');});
		});
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Marcar como spam', 'Esta ação fará com que o autor seja banido da Comunidade Steam, o comentário marcado seja adicionado como dado de treinamento para que possamos sinalizar comentários semelhantes automaticamente e todos os comentários do autor sejam excluídos. Tem certeza de que deseja continuar?', 'Marcar como spam' );
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Inscrever-se no tópico', 'Você receberá uma notificação de comentário quando alguém responder a este tópico.');
			}, function() {
				ShowAlertDialog('Inscrever-se no tópico', 'Houve um problema ao atualizar a sua inscrição. Tente novamente mais tarde.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Cancelar inscrição', 'Você não receberá mais notificações de novas respostas neste tópico.');
			}, function() {
				ShowAlertDialog('Cancelar inscrição', 'Houve um problema ao atualizar a sua inscrição. Tente novamente mais tarde.');
			});
		});
