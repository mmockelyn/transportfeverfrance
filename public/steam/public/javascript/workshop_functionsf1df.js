...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Közzététel dátuma' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Ekkortól:' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Eddig:' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Legutóbbi frissítés dátuma'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Ekkortól:' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Eddig:' } ) );
	...
	var dialog = ShowConfirmDialog( 'Szűrés dátum szerint', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Cégnév';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Keresztnév (ahogy a banki adatoknál szerepel)';
	}
}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Kérjük, adj meg egy cégnevet.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Kérjük, adj meg egy érvényes keresztnevet.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Kérjük, adj meg egy érvényes vezetéknevet.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Kérjük, adj meg egy érvényes címet.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Kérjük, adj meg egy érvényes várost.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Kérjük, adj meg egy érvényes irányítószámot.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Kérjük, add meg telefonszámod a körzetszámmal együtt.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Mentve!', response.redirect_url.length != 0 ? 'Kapcsolattartó- és címinformációd mentésre került. Mivel frissítetted országodat, most átirányításra fogsz kerülni egy külső szolgáltatáshoz adózási információid frissítésére.' : 'Az elérhetőségeid mentésre kerültek. Ha még nem tetted, kérjük, töltsd ki bankszámla- és adózási információidat.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Hiba', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Hiba', 'Ismeretlen hiba lépett fel a kapcsolatinformációid mentése közben.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Kérjük, adj meg egy érvényes e-mail-címet.<br/>';
		}
		...
		{
			errorString += 'Kérjük, adj meg egy valós banknevet.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Kérjük, adj meg egy érvényes bankszámla-tulajdonos nevet (ahogy a banknál szerepel).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Kérjük, add meg a bankod érvényes címét.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Kérjük, add meg a bankod érvényes városát.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Kérjük, add meg a bankod érvényes irányítószámát.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Kérjük add meg bankszámlaszámod.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Kérjük, add meg a bankod routing number kódját (9 számjegy).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Kérjük, add meg a Canadian Transit Numbert (9 számjegy, elején 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Kérjük, add meg bankod IBAN kódját.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Kérjük, add meg bankod SWIFT kódját.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Közreműködők kezelése', 'Barátok listájának betöltése' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Közreműködők kezelése', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Mégse' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Hiba', 'A megadott profil nem található.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Biztosan véglegesíted a közreműködőket? E lépést követően nem fogod tudni hozzáadni vagy törölni őket.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Kérjük, várj.', 'Lehetőségek betöltése...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Válassz szolgáltatókat';
		var strSaveChanges = 'Folytatás';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Hiba', 'A szolgáltatók bevételi százalékainak összesen 100-nak kell lenniük.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Hiba', 'Nem választhatsz olyan külső szervezetet, ami egyúttal közreműködő is az elemednél.' );
				break;
			default:
				ShowAlertDialog( 'Hiba', 'Hiba lépett fel a kérésed feldolgozása közben:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
