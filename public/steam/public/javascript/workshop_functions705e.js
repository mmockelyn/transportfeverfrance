...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Ngày đăng' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Từ' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Đến' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Ngày cập nhật gần nhất'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Từ' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Đến' } ) );
	...
	var dialog = ShowConfirmDialog( 'Lọc theo ngày', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Tên công ty';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Tên (như đã ghi trên dữ liệu ngân hàng)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Số định tuyến';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Số chuyển tiếp tại Canada';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Vui lòng điền tên công ty.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Vui lòng điền tên hợp lệ.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Vui lòng điền họ hợp lệ.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Vui lòng điền địa chỉ hợp lệ.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vui lòng nhập thành phố hợp lệ.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Vui lòng nhập mã bưu điện hoặc mã zip hợp lệ.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Vui lòng điền số điện thoại của bạn, bao gồm cả mã vùng.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Đã lưu!', response.redirect_url.length != 0 ? 'Địa chỉ liên hệ và thông tin liên lạc của bạn đã được lưu. Vì bạn đã cập nhật quốc gia, bạn sẽ được điều hướng tới một trang dịch vụ bên ngoài để cập nhật thông tin thuế của bạn.' : 'Thông tin cá nhân và địa chỉ của bạn đã được lưu. Nếu bạn chưa cung cấp tài khoản ngân hàng và thông tin thuế, xin hãy điền vào đây.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Lỗi', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Lỗi', 'Một lỗi không xác định đã xảy ra khi đang lưu thông tin liên lạc của bạn.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Vui lòng nhập địa chỉ email hợp lệ.<br/>';
		}
		...
		{
			errorString += 'Vui lòng nhập một tên ngân hàng hợp lệ.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vui lòng nhập tên chủ khoản hợp lệ (phải trùng với dữ liệu ngân hàng).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vui lòng nhập một địa chỉ hợp lệ cho ngân hàng của bạn.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Vui lòng nhập thành phố hợp lệ cho ngân hàng của bạn.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Vui lòng nhập mã zip hoặc mã bưu điện hợp lệ cho ngân hàng của bạn.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Vui lòng nhập số tài khoản ngân hàng của bạn.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Vui lòng nhập số định tuyến ngân hàng của bạn (9 chữ số).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Vui lòng nhập số chuyển tiếp tại Canada (9 chữ số bắt đầu bằng 0)<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Vui lòng điền IBAN của ngân hàng của bạn.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Vui lòng nhập mã SWIFT của ngân hàng của bạn.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Quản lý người đóng góp', 'Đang tải danh sách bạn bè' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Quản lý người đóng góp', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Hủy' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Lỗi', 'Không thể tìm thấy hồ sơ chỉ định.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Bạn có chắc muốn chốt danh sách người đóng góp? Bạn sẽ không thể thêm hoặc bỏ họ ra khỏi danh sách sau bước này.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Vui lòng đợi', 'Đang nạp tùy chọn...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Chọn nhà cung cấp dịch vụ';
		var strSaveChanges = 'Tiếp tục';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Lỗi', 'Tỷ lệ chia doanh thu của nhà cung cấp dịch vụ phải có tổng bằng 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Lỗi', 'Bạn không thể chọn một tổ chức thứ ba khi họ đã là người đóng góp cho vật phẩm của bạn.' );
				break;
			default:
				ShowAlertDialog( 'Lỗi', 'Đã có lỗi xảy ra trong quá trình xử lí yêu cầu của bạn:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
