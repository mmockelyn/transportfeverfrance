...
var CForum = Class.create( {
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Παρουσιάστηκε ένα σφάλμα κατά τη δημιουργία νέου θέματος:' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Συνδρομή στο φόρουμ', 'Παρουσιάστηκε ένα σφάλμα κατά τη συνδρομή σε αυτό το φόρουμ.', 'Παρακαλούμε δοκιμάστε ξανά αργότερα.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Κατάργηση συνδρομής φόρουμ', 'Παρουσιάστηκε ένα σφάλμα κατά την κατάργηση της συνδρομής από αυτό του φόρουμ.', 'Παρακαλούμε δοκιμάστε ξανά αργότερα.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
{
	ShowConfirmDialog('Μόνιμη διαγραφή ',
...
{
	ShowConfirmDialog('Σημείωση ως spam ',
		'Αυτό θα διαγράψει τις επιλεγμένες συζητήσεις και θα αποκλείσει τους συγγραφείς τους από το φόρουμ. Είστε σίγουροι ότι θέλετε να κάνετε κάτι τέτοιο;',
		'Διαγραφή spam'
	).done( function() {
...
CForumTopic = Class.create( {
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Αποτυχία επεξεργασίας θέματος. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			...
			var fnFail = ShowForumSuccessDialog.bind( null, 'Συνδρομή στη συζήτηση', 'Λυπούμαστε, δεν ήταν δυνατό να εγγραφείτε σε αυτή τη συζήτηση. Παρακαλούμε δοκιμάστε ξανά αργότερα.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			...
			var fnFail = ShowForumSuccessDialog.bind( null, 'Κατάργηση συνδρομής', 'Λυπούμαστε, δεν μπορέσαμε να σας διαγράψουμε από αυτή τη συζήτηση. Παρακαλούμε δοκιμάστε ξανά αργότερα.');
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Μόνιμη διαγραφή ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Μόνιμη διαγραφή', 'Τα επιλεγμένα θέματα έχουν διαγραφεί οριστικά.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Σημείωση ως spam', 'Τα επιλεγμένα θέματα διαγράφθηκαν και οι δημιουργοί τους αποκλείστηκαν.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Σφάλμα', 'Υπήρξε ένα σφάλμα κατά την επεξεργασία όλου ή τμήματος του αιτήματός σας. Παρακαλούμε επιβεβαιώστε ότι έχετε τα αναμενόμενα αποτελέσματα.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
			...
			Modal.done( function() {
				...
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Αποκλεισμός χρήστη', ban_response.message ? ban_response.message : 'Μπορεί να μην έχετε άδεια να αποκλείετε χρήστες. Επιβεβαιώστε τις άδειες του λογαριασμού σας ή προσπαθήστε ξανά αργότερα.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Προειδοποίηση χρήστη', ban_response.message ? ban_response.message : 'Αυτός ο χρήστης προειδοποιήθηκε με επιτυχία για την ανάρτησή του.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Προειδοποίηση χρήστη', ban_response.message ? ban_response.message : 'Υπήρξε ένα σφάλμα κατά την προειδοποίηση αυτού του χρήστη, παρακαλούμε δοκιμάστε ξανά αργότερα.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Αποκλεισμός χρήστη', 'Μπορεί να μην έχετε άδεια να αποκλείετε χρήστες. Επιβεβαιώστε τις άδειες του λογαριασμού σας ή προσπαθήστε ξανά αργότερα.' );
		}
		else {
			ShowAlertDialog( 'Προειδοποίηση χρήστη', 'Υπήρξε ένα σφάλμα κατά την προειδοποίηση αυτού του χρήστη, παρακαλούμε δοκιμάστε ξανά αργότερα.' );
		}
	});
}
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Σφάλμα', 'Υπήρξε σφάλμα κατά την αποστολή του προσωπικού μηνύματος. Παρακαλούμε δοκιμάστε ξανά.' + '(' + response.success + ')' );
				}
			});
	} );
