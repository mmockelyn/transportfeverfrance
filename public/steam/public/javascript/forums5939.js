...
{
	ShowConfirmDialog('Markera som skräppost ',
...
CForumTopic = Class.create( {
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			...
			var fnFail = ShowForumSuccessDialog.bind( null, 'Sluta prenumerera på diskussion', 'Du kunde tyvärr inte avbryta prenumerationen på den här diskussionen. Var god försök igen senare.');
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Markera som skräppost', 'De valda ämnena har tagits bort och skaparna har stängts av.' );
		});
});
}
