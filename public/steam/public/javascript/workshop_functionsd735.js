...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Data pubblicazione' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Tra' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'e' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Data ultimo aggiornamento'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Tra' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'e' } ) );
	...
	var dialog = ShowConfirmDialog( 'Filtra per data', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Nome della società';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Nome (come scritto sui documenti della banca)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Numero di instradamento';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Numero di transito canadese';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Inserisci il nome di un\'azienda.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Inserisci un nome valido.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Inserisci un cognome valido.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Inserisci un indirizzo valido.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Inserisci una città valida.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Inserisci un CAP o codice postale valido.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Inserisci il tuo numero di telefono. Non omettere il prefisso.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Salvato!', response.redirect_url.length != 0 ? 'L\'indirizzo ed i tuoi contatti sono stati salvati. Dato che hai modificato il tuo Paese sarai ora reindirizzato su una pagina esterna per aggiornare i tuoi dati fiscali.' : 'I tuoi dati di contatto e l\'indirizzo sono stati salvati. Se non l\'hai già fatto, compila i dati bancari e fiscali.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Errore', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Errore', 'Si è verificato un errore inaspettato durante il salvataggio delle tue informazioni di contatto.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Inserisci un indirizzo email valido.<br/>';
		}
		...
		{
			errorString += 'Inserisci un nome valido per tua banca.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Inserisci un nome valido per il titolare del conto bancario (deve corrispondere a quello fornito alla banca).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Inserisci un indirizzo valido per la tua banca.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Inserisci una città valida per la tua banca.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Inserisci un CAP o codice postale valido per la tua banca.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Inserisci il numero del tuo conto bancario.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Inserisci il numero di instradamento della tua banca (9 cifre).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Inserisci il numero di transito canadese (9 cifre con uno 0 iniziale).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Inserisci il codice IBAN della tua banca.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Devi inserire il codice SWIFT della tua banca<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Gestisci i collaboratori', 'Caricamento della lista amici' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Gestisci i collaboratori', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Annulla' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Errore', 'Impossibile trovare il profilo specificato.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Sei sicuro di voler finalizzare i collaboratori? Non potrai più aggiungerne o rimuoverne dopo questo passo.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Attendi', 'Caricamento delle opzioni...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Seleziona il fornitore di servizi';
		var strSaveChanges = 'Continua';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Errore', 'La percentuale di guadagno del fornitore di servizio deve equivalere a 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Errore', 'Non puoi scegliere un\'organizzazione di terze parti che svolge anche il ruolo di normale contributore del tuo oggetto.' );
				break;
			default:
				ShowAlertDialog( 'Errore', 'Si è verificato un errore durante l\'elaborazione della tua richiesta:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
