...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Дата на публикуване' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'между' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'и' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Дата на последно обновяване'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'между' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'и' } ) );
	...
	var dialog = ShowConfirmDialog( 'Филтриране по дата', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Име на компанията';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Собствено име (както е изписано в банковите записи)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Маршрутизационен номер';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Канадски транзитен номер';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Моля, въведете името на компанията.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Моля, въведете валидно собствено име.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Моля, въведете валидно фамилно име.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Моля, въведете валиден адрес.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Моля, въведете валиден град.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Моля, въведете валиден пощенски код.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Моля, въведете телефонния си номер, включвайки и кода на областта.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Запазена!', response.redirect_url.length != 0 ? 'Информацията за връзка и адресът Ви бяха запазени. Тъй като сте обновили своята държава, сега ще бъдете пренасочени към външна услуга, така че да направите същото с данъчната си информация.' : 'Информацията за връзка и адресът Ви бяха запазени. Моля, въведете банковата сметка и данъчната си информация, ако все още не сте.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Грешка', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Грешка', 'Възникна неизвестна грешка при опита да се запази информацията Ви за връзка.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Моля, въведете валидна е-поща.<br/>';
		}
		...
		{
			errorString += 'Моля, въведете валидно име на банка.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Моля, въведете валиден титуляр по сметката (трябва да съответства на банковите записи).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Моля, въведете валиден адрес на Вашата банка.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Моля, въведете валиден град за банката Ви.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Моля, въведете валиден пощенски код за банката Ви.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Моля, въведете номера на Вашата банкова сметка.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Моля, въведете маршрутизационен номер на Вашата банка (9 цифри).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Моля, въведете канадския транзитен номер (9 цифри с водеща 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Моля, въведете IBAN номера на Вашата банка.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Моля, въведете SWIFT кода на Вашата банка.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Управляване на сътрудници', 'Зареждане на списъка с приятели' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Управляване на сътрудници', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Отказ' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Грешка', 'Посоченият профил не може да бъде намерен.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Сигурни ли сте, че искате да финализирате сътрудниците? Ще сте неспособни да ги добавите или премахнете след тази стъпка.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Моля, изчакайте', 'Зареждане на опции…' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Изберете доставчици на услуги';
		var strSaveChanges = 'Продължаване';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Грешка', 'Процентът приходи за доставчика на услуги трябва да са общо до 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Грешка', 'Не можете да изберете организация, която също е нормално сътрудник на Вашата артикул.' );
				break;
			default:
				ShowAlertDialog( 'Грешка', 'Възникна грешка, докато обработвахме заявката Ви:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
