...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Data de publicação' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Data da últ. atualização'} ) );
	...
	var dialog = ShowConfirmDialog( 'Filtrar por data', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Nome da Empresa';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Primeiro nome (como escrito nos registos bancários)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
		...
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Número de Trânsito Canadiano';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Por favor introduza o nome de uma empresa.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'O primeiro nome tem de ser válido.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'O último nome tem de ser válido.<br/>';
				}
			}
		}
		...
		{
			errorString += 'O endereço tem de ser válido.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'A cidade tem de ser válida.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'O código postal tem de ser válido.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Por favor, introduz o teu n.º de telemóvel, incluindo o indicativo.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Dados guardados!', response.redirect_url.length != 0 ? 'As suas informações de contacto e morada foram guardadas. Como atualizou o seu país, será agora redirecionado(a) para um serviço externo para atualizar as suas informações fiscais.' : 'Os dados de contacto foram guardadas com sucesso. E já agora, também recomendamos que as informações bancárias e fiscais sejam preenchidas o mais rápido possível.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Erro', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Erro', 'Ocorreu um erro desconhecido ao tentar guardar as informações de contacto.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'O endereço de e-mail tem de ser válido.<br/>';
		}
		...
		{
			errorString += 'O nome do banco tem de ser válido.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'O nome do titular da conta bancária tem de ser válido (tem de corresponder aos registos bancários).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'O endereço do banco tem de ser válido.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'A cidade do banco tem de ser válida.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'O código postal do banco tem de ser válido.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Por favor, introduz o número da tua conta bancária.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Por favor, introduz o routing number do teu banco (9 dígitos).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Por favor introduza o Número de Trânsito Canadiano (9 dígitos começando com um 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Por favor, introduz o IBAN do teu banco.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Por favor introduza o código SWIFT do seu banco.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Gerir colaboradores', 'A Carregar Lista de Amigos' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Gerir colaboradores', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Cancelar' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Erro', 'O perfil especificado não pôde ser encontrado.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Tens a certeza de que queres confirmar estes colaboradores? Não poderás adicionar mais ou removê-los depois deste passo.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Por favor, aguarda', 'A carregar opções...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Seleciona prestadores de serviço';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Erro', 'As percentagens de receita do prestador de serviço devem totalizar até 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Erro', 'Não podes escolher uma organização de terceiros que também seja um contribuidor normal no teu item.' );
				break;
			default:
				ShowAlertDialog( 'Erro', 'Foi encontrado um erro ao processar o pedido' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
