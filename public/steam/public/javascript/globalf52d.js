...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( 'Αναφορά παράβασης', 'Υπήρξε ένα πρόβλημα με την αποθήκευση της αναφοράς σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Σημείωση: το URL αυτό δεν είναι επίσημη ιστοσελίδα του Steam.\n\n'
						...
						+ 'Θέλετε σίγουρα να επισκεφθείτε αυτή τη σελίδα; Πατήστε OK για να συνεχίσετε με δικό σας ρίσκο.\n' );
	}
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Αλλαγή γλώσσας', 'Υπήρξε ένα πρόβλημα κατά την επικοινωνία με τους διακομιστές του Steam. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
		});
}
	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Το περιεχόμενο μπορεί να μην είναι κατάλληλο για όλες τις ηλικίες', data['html'], 'Προβολή περιεχομένου' );
				...
				{
					ShowAlertDialog('Το περιεχόμενο μπορεί να μην είναι κατάλληλο για όλες τις ηλικίες', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Προβολή ειδοποίησης για περιεχόμενο ενηλίκων' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Αφαίρεση θόλωσης για αυτό το περιεχόμενο μόνο' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Θόλωση αυτού του περιεχομένου' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Το περιεχόμενο ενδέχεται να μην είναι κατάλληλο για όλους'} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Συνδρομή στη συζήτηση', 'Υπήρξε ένα πρόβλημα με την ενημέρωση της συνδρομής σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Συνδρομή στη συζήτηση', 'Υπήρξε ένα πρόβλημα με την ενημέρωση της συνδρομής σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Παρακαλούμε εισάγετε ένα σχόλιο για ανάρτηση.' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Επίσημανση ανεπιθύμητου', 'Αυτό θα δώσει έναν αποκλεισμό κοινότητας στον συντάκτη, θα προσθέσει αυτό το σχόλιο ως δεδομένα εκμάθησης για να μπορούμε να επισημάνουμε τέτοιου είδους σχόλια αυτόματα και θα εκκαθαρίζει όλα τα σχόλια του συντάκτη. Είστε σίγουροι ότι αυτό θέλετε να κάνετε;', 'Επίσημανση ανεπιθύμητου' );
		...
		$modal.done( function(){
			...
			new Ajax.Request( _$this.GetActionURL( 'updateanswer' ), {
				...
				onFailure: function( transport ) {
					if ( transport.responseJSON && transport.responseJSON.success )
					{
						var strError = 'Υπήρξε ένα πρόβλημα κατά την ενημέρωση της απάντησης αυτού του θέματος. Σφάλμα:' + transport.responseJSON.success;
	...
	{
		var strMessage = 'Σας ζητούμε συγγνώμη, κάποιο σφάλμα προέκυψε: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'Υπήρξε ένα σφάλμα κατά την επικοινωνία με το δίκτυο. Παρακαλούμε δοκιμάστε ξανά αργότερα.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				...
				ShowAlertDialog('Συνδρομή στη συζήτηση', 'Υπήρξε ένα πρόβλημα με την ενημέρωση της συνδρομής σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.');
			});
		});
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				...
				ShowAlertDialog('Κατάργηση συνδρομής', 'Υπήρξε ένα πρόβλημα με την ενημέρωση της συνδρομής σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.');
			});
		});
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onFailure: function(transport) {
			ShowAlertDialog( 'Κοινοποίηση', 'Παρουσιάστηκε ένα πρόβλημα με την κοινοποίηση της κατάστασής σας. Παρακαλούμε δοκιμάστε ξανά αργότερα.' );
		}
	});
}
