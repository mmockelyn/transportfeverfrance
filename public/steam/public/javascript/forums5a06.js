...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Nếu kho đồ của bạn ở chế độ công khai, mọi thành viên khác sẽ có thể gửi cho bạn đề nghị trao đổi ở chủ đề này.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>Kho đồ của bạn được đặt ở chế độ công khai</b>. Điều này có nghĩa là các thành viên trong cộng đồng được gửi đề nghị trao đổi tới bạn từ chủ đề này. Nếu bạn thiết lập chế độ riêng tư cho rương của mình khác với chế độ công khai, thì bạn sẽ không nhận được các đề nghị trao đổi từ cộng đồng.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>Kho đồ của bạn <u>không</u> ở chế độ công khai</b>. Điều này có nghĩa là các thành viên trong cộng đồng sẽ không thể gửi đề nghị trao đổi tới bạn từ chủ đề này. Nếu bạn chuyển thiết lập rương của mình sang chế độ công khai, các thành viên trong cộng đồng sẽ có thể gửi đề nghị trao đổi tới bạn từ chủ đề này.';
			...
			strMessage += 'Khi bạn kết thúc quá trình nhận đề nghị trao đổi hoặc đã trao đổi xong, bạn có thể đóng vấn đề này hoặc cấm trao đổi nữa.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Đã có lỗi xảy ra khi tạo một chủ đề mới: ' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Đăng ký diễn đàn', 'Đã có lỗi xảy ra khi đăng ký theo dõi diễn đàn này.', 'Xin vui lòng thử lại sau.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Hủy đăng ký diễn đàn', 'Đã có lỗi xảy ra khi hủy theo dõi diễn đàn này.', 'Xin vui lòng thử lại sau.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Xóa chủ đề ',
				'Bạn có thật sự muốn xóa chủ đề này? ' +
				'Hành động này chỉ có thể được khôi phục lại bởi một người điều hành.',
				'Xóa chủ đề'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Xóa vĩnh viễn ',
		'Bạn có chắc muốn xóa chủ đề này vĩnh viễn? Hành động này không thể hoàn tác được.',
		'Xóa vĩnh viễn'
	).done( function() {
...
{
	ShowConfirmDialog('Gỡ bỏ đánh đấu nội dung có thể bị cấm ',
		'Bạn có chắc muốn gỡ bỏ đánh dấu nội dung có thể bị cấm cho chủ đề này? Việc này không thể hoàn tác.',
		'Gỡ bỏ đánh đấu nội dung có thể bị cấm'
	).done( function() {
...
{
	ShowConfirmDialog('Cấm người bình luận ',
	'Bạn có chắc mình muốn cấm những người dùng đã bình luận quá 5 lần trong chủ đề này? Hành động này sẽ rất khó hoàn tác.',
	'Cấm người bình luận'
).done( function() {
...
{
	ShowConfirmDialog('Đánh dấu spam ',
		'Hành động này sẽ xóa các chủ đề đã chọn và cấm tác giả khỏi diễn đàn. Bạn có chắc muốn thực hiện?',
		'Xóa spam'
	).done( function() {
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						'Kết hợp chủ đề',
						'Chủ đề được lựa chọn đã được sáp nhập vào một chủ đề duy nhất.',
						'Tới chủ đề đã gộp',
...
{
	ShowConfirmDialog( 'Đóng đề tài này', 'Đánh dấu "Đóng" trong topic này bạn sẽ không thể trả lời hay đề nghị trao đổi nữa. Một khi nó đóng, bạn sẽ không thể làm gì được hết.', 'Đóng đề tài này')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Tố cáo', 'Cảm ơn bạn đã gửi báo cáo', 'Báo cáo của bạn đã được gửi đến những người kiểm duyệt để xem xét. Chúng tôi sẽ cho bạn biết nếu tiến hành biện pháp xử lý phù hợp.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Sửa chủ đề thất bại. Thông điệp lỗi:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Chỉnh sửa chủ đề không thành công. Xin vui lòng thử lại sau.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Theo dõi thảo luận', 'Bạn đã đăng ký theo dõi thảo luận này.', 'Bạn sẽ nhận được một lời nhắn khi ai đó gửi câu trả lời tới thảo luận này.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Theo dõi thảo luận', 'Xin thứ lỗi, chúng tôi không thể đăng ký cho bạn theo dõi cuộc thảo luận này. Xin vui lòng thử lại sau.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Hủy theo dõi thảo luận', 'Bạn đã hủy theo dõi thảo luận này.', 'Bạn sẽ không còn nhận thông báo bình luận từ thảo luần này.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Hủy theo dõi thảo luận', 'Xin thứ lỗi, chúng tôi không thể hủy đăng ký theo dõi của bạn cho cuộc thảo luận này. Xin vui lòng thử lại sau.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Những báo cáo có liên quan đến chủ đề này đều đã được đánh dấu là đã giải quyết.<br>số báo cáo cần giải quyết: ' + cReportsResolved;
				else
					strMessage = 'Những báo cáo có liên quan đến chủ đề này đều đã được đánh dấu là đã giải quyết.';

				ShowForumSuccessDialogWithDetailTitle( 'Giải quyết bài báo cáo', 'Báo cáo đã được giải quyết.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Lịch sử bài viết', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Bạn có thật sự muốn xóa bài viết này?', 'Xóa' )
...
var g_rgRedirectTimeOptions = [
	{value: 0, label: 'Không có đường dẫn' },
	{value: 1, label: '1 giờ' },
	{value: 24, label: '1 ngày' },
	{value: 168, label: '1 tuần' },
	{value: 720, label: '1 tháng' },
	{value: -1, label: 'Mãi mãi' }
];
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'Hợp nhất các chủ đề vào chung với nhau sẽ di chuyển tất cả các bài đăng từ chủ đề này sang chủ đề kia. Nếu trong chủ để được hợp nhất đã có bài đăng, các bài đăng sẽ được hợp nhất xen kẽ dựa theo thời điểm được đăng. Hành động này không thể hoàn tác được.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Chọn một chủ đề được xem gần đây để kết hợp các chủ đề được lựa chọn vào:' : 'Chọn một chủ đề được xem gần đây kết hợp vào chủ đề này:';
	...
	var strMoreInstructions = 'Nếu chủ đề bạn muốn kết hợp không được liệt kê ở đây, bạn có thế <%1$s>duyệt các chủ đề bạn muốn<%2$s> trong một cửa sổ mới. Danh sách này sẽ được tự động cập nhật khi bạn quay trở lại.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Để lại chuyển hướng cho: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Kết hợp các chủ đề lại với nhau thành một chủ đề lâu đời nhất' );
	...
	var Modal = ShowConfirmDialog( 'Kết hợp chủ đề', $DialogContent, 'Kết hợp chủ đề' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Hủy xóa', 'Chủ đề được lựa chọn đã được khôi phục.' );
			else
				ShowAlertDialog( 'Xóa', 'Chủ đề được lựa chọn đã bị xóa.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Mở khóa', 'Những chủ đề được chọn đã được mở khóa.' );
			else
				ShowAlertDialog( 'Khóa', 'Những chủ đề được chọn đã được khóa.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Di chuyển', 'Chủ đề được lựa chọn đã được chuyển đến diễn đàn mới.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Kết hợp chủ đề',
					'Chủ đề được lựa chọn đã được sáp nhập vào một chủ đề duy nhất.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Xóa vĩnh viễn ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Xóa vĩnh viễn', 'Chủ đề được chọn đã bị xóa vĩnh viễn.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Đánh dấu spam', 'Chủ đề được chọn đã bị xóa và người tạo nó đã bị cấm.' );
		});
});
}
...
{
	ShowConfirmDialog('Cấm người bình luận ',
	'Hành động này sẽ ban hành một lệnh cấm cộng đồng (hoặc đánh dấu hijack lạc đề) cho người dùng đã bình luận nhiều hơn một mức định ra trên các chủ đề được chọn. Bạn có chắc mình muốn làm như thế không?',
	'Cấm người bình luận'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Cấm người bình luận', 'Những người dùng này bị cấm do đã bình luận trên hạn mức trong các chủ đề được chọn.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Lỗi', 'Đã có lỗi xảy ra trong quá trình xử lý tất cả hoặc một phần yêu cầu của bạn. Xin vui lòng xác minh rằng bạn đã được nhận kết quả mong đợi.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Cảnh cáo người dùng', $WaitElem, 'Cảnh cáo người dùng' );
	} else {
		var Modal = ShowConfirmDialog( 'Cấm người dùng', $WaitElem, 'Cấm người dùng' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Cấm người dùng', 'Phải có ít nhất một lựa chọn. Người dùng đã không bị cấm.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Cấm người dùng', ban_response.message ? ban_response.message : 'Quyền đăng và chỉnh sửa của người dùng này đã bị thu hồi.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Cấm người dùng', ban_response.message ? ban_response.message : 'Bạn không có quyền cấm người dùng. Xin vui lòng xác thực lại quyền hành tài khoản của bạn hoặc thử lại sau.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Cảnh cáo người dùng', ban_response.message ? ban_response.message : 'Đã cảnh cáo thành công người dùng này vì bài viết của họ.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Cảnh cáo người dùng', ban_response.message ? ban_response.message : 'Chúng tôi gặp phải lỗi khi cảnh cáo người dùng này, vui lòng thử lại sau.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Cấm người dùng', 'Bạn không có quyền cấm người dùng. Xin vui lòng xác thực lại quyền hành tài khoản của bạn hoặc thử lại sau.' );
		}
		else {
			ShowAlertDialog( 'Cảnh cáo người dùng', 'Chúng tôi gặp phải lỗi khi cảnh cáo người dùng này, vui lòng thử lại sau.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Vui lòng nhập tin nhắn bạn muốn gửi riêng cho người dùng này:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Bài viết này sẽ tự động liên kết tới tin nhắn riêng của bạn.</div>';

	var $modal = ShowConfirmDialog( 'Tin nhắn riêng', $strDialogHTML, 'Gửi tin nhắn riêng' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Tin nhắn riêng', response.message );
				}
				else
				{
					ShowAlertDialog( 'Lỗi', 'Đã có lỗi xảy ra trong khi gửi tin nhắn riêng. Vui lòng thử lại sau.' + '(' + response.success + ')' );
				}
			});
	} );
