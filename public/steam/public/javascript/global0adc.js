...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( 'รายงานการละเมิด', 'ตรวจพบปัญหาในการบันทึกรายงานของคุณ กรุณาลองใหม่อีกครั้งภายหลัง' );
	});
	return false;
}
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'เปลี่ยนภาษา', 'ตรวจพบปัญหาในการติดต่อสื่อสารกับเซิร์ฟเวอร์ Steam กรุณาลองใหม่อีกครั้งภายหลัง' );
		});
}
	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('เนื้อหาอาจไม่เหมาะสมสำหรับทุกวัย', data['html'], 'ดูเนื้อหา' );
				...
				{
					ShowAlertDialog('เนื้อหาอาจไม่เหมาะสมสำหรับทุกวัย', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'ดูคำเตือนเนื้อหาสำหรับผู้ใหญ่' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'ลบการเบลอสำหรับเนื้อหานี้เท่านั้น' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'เบลอเนื้อหานี้' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'เนื้อหาอาจไม่เหมาะสมสำหรับทุกวัย'} ) );
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('สมัครสมาชิกความเห็นนี้', 'ตรวจพบปัญหาในการอัปเดตการสมัครสมาชิกของคุณ กรุณาลองใหม่อีกครั้งภายหลัง');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('สมัครสมาชิกความเห็นนี้', 'ตรวจพบปัญหาในการอัปเดตการสมัครสมาชิกของคุณ กรุณาลองใหม่อีกครั้งภายหลัง');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'กรุณากรอกความเห็นเพื่อโพสต์' );
			return;
		}
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'ทำเครื่องหมายเป็นสแปม', 'นี่จะเป็นการมอบแบนชุมชนแก่ผู้เขียน การเพิ่มความคิดเห็นนี้เป็นข้อมูลการฝึกอบรม เพื่อให้เราสามารถตั้งค่าสถานะความเห็นประเภทนี้ได้โดยอัตโนมัติ และล้างความเห็นทั้งหมดของผู้เขียน คุณแน่ใจหรือไม่ว่าคุณต้องการจะดำเนินการนี้?', 'ทำเครื่องหมายเป็นสแปม' );
	...
	DisplayError: function( elError, transport )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.error )
			...
			strMessage += 'ตรวจพบข้อผิดพลาดในการติดต่อสื่อสารกับเครือข่าย กรุณาลองใหม่อีกครั้งภายหลัง';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				...
				ShowAlertDialog('สมัครสมาชิกความเห็นนี้', 'ตรวจพบปัญหาในการอัปเดตการสมัครสมาชิกของคุณ กรุณาลองใหม่อีกครั้งภายหลัง');
			});
		});
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				...
				ShowAlertDialog('ยกเลิกการสมัครสมาชิกความเห็นนี้', 'ตรวจพบปัญหาในการอัปเดตการสมัครสมาชิกของคุณ กรุณาลองใหม่อีกครั้งภายหลัง');
			});
		});
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onFailure: function(transport) {
			ShowAlertDialog( 'แบ่งปัน', 'ตรวจพบปัญหาในการแบ่งปันการอัปเดตสถานะของคุณ กรุณาลองใหม่อีกครั้งภายหลัง' );
		}
	});
}
