...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'インベントリのプライバシー設定が「公開」なら、コミュニティのメンバーはこのトピックからあなたにトレードオファーを送信できます。';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>インベントリのプライバシーは公開に設定されています</b>。 これはコミュニティのメンバーがこのトピックからあなたにトレードオファーを送信できるということです。インベントリのプライバシーが公開以外に設定されていると、コミュニティのトレードオファーを受けることができません。';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>インベントリのプライバシーが公開に設定されて<u>いません</u></b>。この状態では、コミュニティメンバーがこのトピックからあなたにトレードオファーを送信できません。インベントリのプライバシー設定が「公開」にすると、コミュニティメンバーはこのトピックからあなたにトレードオファーを送ることができるようになります。';
			...
			strMessage += 'これ以上トレードオファーを受け取りたくないか、既にトレードが完了した時は、このトピックをクローズしここからのトレードオファーを禁止することが出来ます。';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( '新しいトピックの作成中にエラーが発生しました: ' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( '掲示板に登録する', 'この掲示板への登録中にエラーが発生しました。', '後でもう一度お試しください。' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( '掲示板から登録を取り消す', 'この掲示板からの登録解除中にエラーが発生しました。', '後でもう一度お試しください。' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('スレッドを削除 ',
				'このスレッドを削除しますか？ ' +
				'この作業はモデレーターだけが取り消す事ができます。',
				'スレッドを削除'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('完全に削除 ',
		'本当にこのトピックを完全に削除しますか？この操作は元に戻すことが出来ません。',
		'完全に削除'
	).done( function() {
...
{
	ShowConfirmDialog('禁止コンテンツフラグをクリア ',
		'このトピックの禁止コンテンツフラグをクリアしますか？この処理を取り消すことはできません。',
		'禁止コンテンツフラグをクリア'
	).done( function() {
...
{
	ShowConfirmDialog('コメント投稿者を禁止 ',
	'このトピックに5回以上コメントしたユーザーを禁止しますか？禁止の取り消しは非常に困難です。',
	'コメント投稿者を禁止'
).done( function() {
...
{
	ShowConfirmDialog('スパムと判定 ',
		'これにより、選択したスレッドは削除され、投稿者は掲示板で禁止となります。実行してよろしいですか？',
		'スパムを削除'
	).done( function() {
...
{
	ShowConfirmDialog( 'このトピックをクローズ', 'このトピックをクローズとしてマークすると、それ以上返信やトレードオファーができなくなります。クローズされたら、そのトピックにそれ以上アクションを起こすことはできません。', 'このトピックをクローズ')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( '報告', 'ご報告ありがとうございます', 'あなたの報告はモデレーターに審査対象として送信されました。' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'トピックの変更に失敗しました。エラーメッセージ：' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'トピックの編集に失敗しました。時間を置いて再試行してください。' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'ディスカッションに登録', 'このディスカッションに登録されました。', 'このスレッドに誰かが返信する度、通知を受け取ります。' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'ディスカッションに登録', '申し訳ありませんが、このディスカッションに登録できませんでした。後ほどもう一度お試しください。');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( '掲示板から登録解除', 'このスレッドから登録解除されました。', '以後このスレッドのお知らせは受け取りません。' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, '掲示板から登録解除', 'このスレッドから登録を解除できませんでした。後ほどもう一度お試しください。');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'このトピックに関連した全ての報告は既に解決済みとしてマークされています。<br>解決した報告数:' + cReportsResolved;
				else
					strMessage = 'このトピックに関連した全ての報告は既に解決済みとしてマークされています。';

				ShowForumSuccessDialogWithDetailTitle( '通報のあった投稿を解決', '報告が解決されました。', strMessage );
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'この投稿を削除しますか？', '削除' )
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'トピックを統合すると、それらのトピック内の投稿が全て別のトピックへ移されます。統合先のトピックへ既に投稿がある場合、全ての投稿は投稿時間により並べ替えられます。この操作は元に戻すことが出来ません。<br><br>';
	strMessage += rgForumTopics.length > 1 ? '最近閲覧したトピックから選んで、次のトピックに統合する：' : '最近閲覧したトピックから選んで、次のトピックに統合する：';
	...
	var strMoreInstructions = '統合したいトピックがここに表示されていない場合、新しいウィンドウで<%1$s>希望するトピックを閲覧することができます<%2$s>。戻ってくると、リストは自動的に更新されます。'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, '選択されたトピックを最古のトピックに統合' );
	...
	var Modal = ShowConfirmDialog( 'トピックを統合', $DialogContent, 'トピックを統合' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( '復元', '選択したトピックは復元されました。' );
			else
				ShowAlertDialog( '削除', '選択したトピックは削除されました。' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'ロック解除', '選択したトピックのロックが解除されました。' );
			else
				ShowAlertDialog( 'ロック', '選択したトピックはロックされました。' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( '移動...', '選択したトピックは新しい掲示板に移動しました。' );
		});
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('完全に削除 ',
		...
		.done( function( data ) {
			ShowAlertDialog( '完全に削除', '選択されたトピックは完全に削除されました。' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'スパムと判定', '選択したトピックは削除され、作成者は禁止されました。' );
		});
});
}
...
{
	ShowConfirmDialog('コメント投稿者を禁止 ',
	'これにより、選択したスレッドに特定の回数以上コメントしたユーザーにコミュニティ禁止が適用されます（またはハイジャックとマーク）。実行してよろしいですか？',
	'コメント投稿者を禁止'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'コメント投稿者を禁止', '選択したトピックに一定数を超えてコメントしたユーザーは禁止されました。' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'エラー', 'リクエストの全部あるいは一部のプロセスにおいて、エラーが発生しました。希望通りの結果が得られたかをご確認ください。' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'ユーザーに警告', $WaitElem, 'ユーザーに警告' );
	} else {
		var Modal = ShowConfirmDialog( 'ユーザーを接続禁止にする', $WaitElem, 'ユーザーを接続禁止にする' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'ユーザーを接続禁止にする', '少なくとも１つのオプションを選択しなければなりません。 ユーザーは禁止されていません。' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'ユーザーを接続禁止にする', ban_response.message ? ban_response.message : 'このユーザーの投稿と、編集権限は取り消されました。' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'ユーザーを接続禁止にする', ban_response.message ? ban_response.message : 'あなたにはユーザーを禁止するための権限が無いようです。あなたのアカウント権限を確認するか、時間を置いて再試行してください。' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'ユーザーに警告', ban_response.message ? ban_response.message : 'このユーザーの投稿に対する警告が完了しました。' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'ユーザーに警告', ban_response.message ? ban_response.message : 'ユーザーへの警告中にエラーが発生しました。あとでもう一度お試しください。' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'ユーザーを接続禁止にする', 'あなたにはユーザーを禁止するための権限が無いようです。あなたのアカウント権限を確認するか、時間を置いて再試行してください。' );
		}
		else {
			ShowAlertDialog( 'ユーザーに警告', 'ユーザーへの警告中にエラーが発生しました。あとでもう一度お試しください。' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'このユーザーに個人的に送信したいメッセージを入力してください:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*この投稿はあなたのプライベートメッセージで自動的に参照されます。</div>';

	var $modal = ShowConfirmDialog( 'プライベートメッセージ', $strDialogHTML, 'プライベートメッセージを送信' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'プライベートメッセージ', response.message );
				}
				else
				{
					ShowAlertDialog( 'エラー', 'プライベートメッセージの送信中にエラーが発生しました。もう一度お試しください。' + '(' + response.success + ')' );
				}
			});
	} );
