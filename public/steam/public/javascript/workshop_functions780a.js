...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: '投稿日' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: '期間：' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: '最終更新日'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: '期間：' } ) );
	...
	var dialog = ShowConfirmDialog( '日付でフィルター', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = '企業名';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = '名前 (銀行に登録しているものと同じ)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = '支店コード';
		}
		else
		{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( '保存されました', response.redirect_url.length != 0 ? 'あなたの連絡先と住所の情報は保存されました。居住国を変更したため、税金情報の更新を行う外部サービスへと転送されます。' : 'あなたの連絡先と住所の情報は保存されました。 銀行口座と税金情報を入力していない場合は、入力してください。' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'エラー', json.msg );
					}
					else
					{
						ShowAlertDialog( 'エラー', '連絡先情報を保存する際に不明なエラーが発生しました。' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'メールアドレスを正確に入力してください。<br/>';
		}
		...
		if ( $( 'bankcountry' ).value == 'US' )
		{
			...
			{
				errorString += 'ご利用の銀行の支店コードを入力してください（9桁）。<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += '(0から始まる9桁の）Canadian Transit Numberを入力してください。<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'ご利用の銀行のIBANコードを入力してください。<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'ご利用の銀行のSWIFTコードを入力してください。<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( '貢献者の管理', 'フレンドリストをロード中' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( '貢献者の管理', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'キャンセル' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'エラー', '指定されたプロフィールが見つかりませんでした。' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( '貢献者を最終決定してよろしいですか？このステップの後に追加したり削除することはできません。' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'お待ちください', 'オプションをロード中...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'サービスプロバイダの選択';
		var strSaveChanges = '続ける';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'エラー', 'サービス提供者の収益比率は合計100まででなければいけません。' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'エラー', 'すでにあなたのアイテムの通常の貢献者になっているサードパーティの組織は選択することはできません。' );
				break;
			default:
				ShowAlertDialog( 'エラー', 'リクエストの処理中にエラーが発生しました。' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
