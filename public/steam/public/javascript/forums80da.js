...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Hvis din inventarsynlighedsindstilling er offentlig, kan medlemmer af fællesskabet sende dig handelstilbud gennem denne tråd.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>Din inventarsynlighedsindstilling er sat til offentlig</b>. Det betyder, at medlemmer af fællesskabet kan sende handelstilbud fra denne tråd. Hvis du ændrer din inventarsynlighedsindstilling til noget andet end offentlig, kan du ikke modtage handelstilbud fra fællesskabet.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>Din inventarsynlighedsindstilling er <u>ikke</u> sat til offentlig</b>. Det betyder, at medlemmer af fællesskabet ikke kan sende handelstilbud fra denne tråd. Hvis du ændrer din inventarsynlighedsindstilling til offentlig, kan medlemmer af fællesskabet sende dig handelstilbud gennem dette emne.';
			...
			strMessage += 'Når du er færdig med at modtage handelstilbud eller har gennemført handelen, kan du lukke denne tråd for at forhindre nye handelstilbud herfra.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Der opstod en fejl i forsøget på at oprette en ny tråd:' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Abonner på forum', 'Der opstod en fejl i forsøget på at abonnere på dette forum.', 'Prøv venligst igen senere.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Opsig abonnement fra forum', 'Der opstod en fejl i forsøget på af opsige abonnementet fra dette forum.', 'Prøv venligst igen senere.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Slet tråd ',
				'Er du sikker på, at du vil slette denne tråd? ' +
				'Denne handling kan kun fortrydes af en moderator.',
				'Slet tråd'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Slet permanent ',
		'Er du sikker på, at du ønsker at slette denne tråd permanent? Dette kan ikke fortrydes.',
		'Slet permanent'
	).done( function() {
...
{
	ShowConfirmDialog('Fjern markeringen "Forbudt indhold" ',
		'Er du sikker på, at du vil fjerne markeringen om forbudt indhold på emnet? Dette kan ikke fortrydes.',
		'Fjern markeringen "Forbudt indhold"'
	).done( function() {
...
{
	ShowConfirmDialog('Udeluk kommentarer ',
	'Er du sikker på, at du ønsker at udelukke brugere, som har kommenteret mere end 5 gange under dette emne? Det er svært at fortryde denne handling.',
	'Udeluk kommentarer'
).done( function() {
...
{
	ShowConfirmDialog('Markér som spam ',
		'Dette vil slette de valgte tråde og udelukke forfatterne fra forummet. Er du sikker på, at du ønsker dette?',
		'Slet spam'
	).done( function() {
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						'Sammenflet tråde',
						'De valgte tråde er blevet sammenflettet til én tråd.',
						'Gå til den sammenflettede tråd',
						'Tilbage til liste over tråde'
					).done( function() {
...
{
	ShowConfirmDialog( 'Luk denne tråd', 'At markere denne tråd som "Lukket" forhindrer nye svar eller handelstilbud. Når tråden er lukket, kan du ikke foretage yderligere handlinger med den.', 'Luk denne tråd')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Rapporter', 'Tak for din rapportering', 'Din rapportering er blevet sendt til moderatorerne til evaluering. Vi giver dig besked, hvis vi handler på det.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Kunne ikke redigere emnet. Fejlbesked:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Kunne ikke redigere tråden. Prøv venligst igen senere.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Abonner på diskussionen', 'Du abonnerer nu på denne diskussion.', 'Du modtager en notifikation, når nogen svarer på diskussionen.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Abonner på diskussionen', 'Vi beklager, vi var ude af stand til at abonnere på denne diskussion. Prøv venligst igen senere.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Opsig abonnement på diskussion', 'Du abonnerer ikke længere på denne diskussion.', 'Du modtager ikke længere notifikationer fra denne diskussion.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Opsig abonnement på diskussion', 'Vi beklager, vi ikke kan opsige abonnementet på denne diskussion. Prøv igen senere.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Alle rapporter, tilhørende denne tråd, er blevet markeret som "afklaret".<br>Antal rapporter der skulle afklares:' + cReportsResolved;
				else
					strMessage = 'Alle rapporter, tilhørende denne tråd, er allerede blevet markeret som "afklaret".';

				ShowForumSuccessDialogWithDetailTitle( 'Afklar rapporterede indlæg', 'Rapporteringer afklaret.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Indlægshistorik', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Er du sikker på, at du vil slette dette indlæg?', 'Slet' )
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'At sammenflette tråde vil flytte alle indlæg fra én eller flere tråde til en anden tråd. Hvis der allerede er indlæg i den valgte tråd, bliver de nye indlæg indskudt efter tidspunktet, de blev slået op. Dette kan ikke fortrydes.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Vælg en nylig set tråd at sammenflette de valgte tråde ind i:' : 'Vælg en nylig set tråd at sammenflette denne tråd ind i:';
	...
	var strMoreInstructions = 'Hvis tråden, du gerne vil sammenflette til, ikke er angivet her, så kan du <%1$s>lede efter tråden<%2$s> i et nyt vindue. Denne liste opdateres automatisk, når du vender tilbage.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Efterlad omdirigering i: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Sammenflet de valgte tråde i den ældste tråd' );
	...
	var Modal = ShowConfirmDialog( 'Sammenflet tråde', $DialogContent, 'Sammenflet tråde' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Gendan', 'De valgte tråde er blevet gendannet.' );
			else
				ShowAlertDialog( 'Slet', 'De valgte tråde er blevet slettet.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Lås op', 'De valgte tråde er blevet låst op.' );
			else
				ShowAlertDialog( 'Lås', 'De valgte tråde er blevet låst.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Flyt...', 'De valgte tråde er blevet flyttet til det nye forum.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Sammenflet tråde',
					'De valgte tråde er blevet sammenflettet til én tråd.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Slet permanent ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Slet permanent', 'De valgte tråde er blevet permanent slettet.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Markér som spam', 'De valgte emner er slettet og skaberne udelukket.' );
		});
});
}
...
{
	ShowConfirmDialog('Udeluk kommentarer ',
	'Dette tildeler en fællesskabsudelukkelse (eller en markering som hijacket) til brugere, som har kommenteret på de valgte tråde mere end et bestemt antal gange. Er du sikker på, at du ønsker at fortsætte?',
	'Udeluk kommentarer'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Udeluk kommentarer', 'Brugere, som har kommenteret mere end tilladt under de valgte emner, er blevet udelukket.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Fejl', 'Der opstod en fejl under behandlingen af hele eller dele af din anmodning. Kontroller venligst at du fik det forventede resultat.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Advar bruger', $WaitElem, 'Advar bruger' );
	} else {
		var Modal = ShowConfirmDialog( 'Udeluk bruger', $WaitElem, 'Udeluk bruger' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Udeluk bruger', 'Der skal vælges mindst én mulighed. Brugeren er ikke blevet udelukket.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Udeluk bruger', ban_response.message ? ban_response.message : 'Denne brugers indlægs- og redigerinsgsprivilegier er blevet ophævet.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Udeluk bruger', ban_response.message ? ban_response.message : 'Du har ikke tilladelse til at udelukke brugere. Bekræft venligst din kontos tilladelser eller prøv igen senere.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Advar bruger', ban_response.message ? ban_response.message : 'Denne bruger er blevet advaret for sit indlæg.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Advar bruger', ban_response.message ? ban_response.message : 'Vi kunne ikke advare denne bruger. Prøv venligst igen senere.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Udeluk bruger', 'Du har ikke tilladelse til at udelukke brugere. Bekræft venligst din kontos tilladelser eller prøv igen senere.' );
		}
		else {
			ShowAlertDialog( 'Advar bruger', 'Vi kunne ikke advare denne bruger. Prøv venligst igen senere.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Indtast venligst den private besked, du ønsker at sende til denne bruger:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Der vil automatisk blive henvist til dette indlæg i din private besked.</div>';

	var $modal = ShowConfirmDialog( 'Privat besked', $strDialogHTML, 'Send privat besked' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Privat besked', response.message );
				}
				else
				{
					ShowAlertDialog( 'Fejl', 'Kunne ikke sende denne private besked. Prøv igen senere.' + '(' + response.success + ')' );
				}
			});
	} );
