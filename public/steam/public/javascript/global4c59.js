	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('İçerik her yaş grubuna uygun olmayabilir', data['html'], 'İçeriği Görüntüle' );
				...
				{
					ShowAlertDialog('İçerik her yaş grubuna uygun olmayabilir', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Yetişkin İçerik Uyarısını Görüntüle' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Bulanıklaştırmayı bu içerik için kaldır' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Bu içeriği bulanıklaştır' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'İçerik tüm kitleler için uygun olmayabilir'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Spam Olarak İşaretle', 'Bu, yorum sahibini yasaklar. Bu tür yorumları otomatik olarak işaretleyebilmemiz ve yazarın tüm yorumlarını temizleyebilmemiz için bu yorumu eğitim verisi olarak ekleyin. Bunu yapmak istediğinize emin misiniz?', 'Spam Olarak İşaretle' );
