...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Fecha de publicación' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Entre el' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'y el' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Fecha de la última actualización'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Entre el' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'y el' } ) );
	...
	var dialog = ShowConfirmDialog( 'Filtrar por fecha', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Nombre de la compañía';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Nombre (como está escrito en los registros bancarios)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Número de identificación bancaria';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Número de Tránsito Canadiense';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Por favor, introduce el nombre de la compañía.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Por favor, introduce un nombre válido.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Por favor, introduce un apellido válido.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Por favor, introduce una dirección válida.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Por favor, introduce una localidad válida.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Por favor, introduce un código postal o zip válido.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Por favor, introduce tu número de teléfono, incluyendo tu código de área.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( '¡Guardado!', response.redirect_url.length != 0 ? 'Tu información de contacto y dirección han sido guardadas. Dado que has actualizado tu país, se te redirigirá a un servicio externo para actualizar tu información sobre impuestos.' : 'Tu información de contacto y dirección han sido guardadas. Si no lo has hecho ya, por favor, cumplimenta tus datos bancarios y fiscales.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Error', 'Se ha producido un error desconocido mientras se intentaba guardar tu información de contacto.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Por favor, introduce una dirección de correo electrónico válida.<br/>';
		}
		...
		{
			errorString += 'Por favor, introduce un nombre de entidad bancaria válido.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Por favor, introduce un nombre de titular de cuenta bancaria válido (debe coincidir con los registros bancarios).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Por favor, introduce una dirección válida para tu entidad bancaria.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Por favor, introduce una localidad válida para tu entidad bancaria.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Por favor, introduce un código postal o zip válido para tu entidad bancaria.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Por favor, introduce el número de tu cuenta bancaria.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Por favor, introduce tu número de identificación bancaria (9 dígitos).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Por favor, introduce el Número de Tránsito Canadiense (9 dígitos con un 0 al principio).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Por favor, introduce el IBAN de tu entidad bancaria.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Por favor, introduce el código SWIFT de tu banco.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Gestionar colaboradores', 'Cargando lista de amigos' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Gestionar colaboradores', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Cancelar' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Error', 'No se ha encontrado el perfil especificado.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( '¿Seguro que quieres establecer estos colaboradores? No podrás añadirlos o eliminarlos tras este paso.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Espera, por favor', 'Cargando opciones...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Seleccionar proveedores de servicio';
		var strSaveChanges = 'Continuar';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Error', 'Los porcentajes de ingresos del proveedor de servicios deben tener un total de hasta 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Error', 'No puedes elegir una organización de terceros que también colabore habitualmente en tu artículo.' );
				break;
			default:
				ShowAlertDialog( 'Error', 'Se ha producido un error mientras se procesaba la solicitud:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
