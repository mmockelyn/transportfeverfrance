	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('모든 연령대에 적절하지 않은 콘텐츠', data['html'], '내용 보기' );
				...
				{
					ShowAlertDialog('모든 연령대에 적절하지 않은 콘텐츠', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( '성인 콘텐츠 경고 보기' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( '이 콘텐츠만 흐림 표시 제거' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( '콘텐츠 흐리게 표시하기' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': '모든 연령대에 적절하지 않은 콘텐츠'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( '스팸으로 표시', '작성자에게 커뮤니티 차단을 적용하고, 이 댓글을 트레이닝 데이터로 추가하여 이러한 유형의 댓글에 자동으로 깃발 표시를 적용하며 작성자의 모든 댓글을 삭제하도록 합니다. 계속 진행할까요?', '스팸으로 표시' );
	...
	UpdateAnswer: function( gidComment, bExisting )
	{
		...
		else
		{
			...
			if ( bExisting )
				strModalBody = '이미 답변으로 채택된 다른 게시물이 있습니다. 이 게시물을 새 답변으로 채택하시겠습니까?';
