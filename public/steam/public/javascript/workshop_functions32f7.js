...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Geplaatst op' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Tussen' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'en' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Laatst bijgewerkt op'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Tussen' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'en' } ) );
	...
	var dialog = ShowConfirmDialog( 'Filteren op datum', content );
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Opgeslagen!', response.redirect_url.length != 0 ? 'Je contact- en adresinformatie is opgeslagen. Omdat je jouw land hebt bijgewerkt word je nu doorgestuurd naar een externe dienst om je belastinginformatie bij te werken.' : 'Je contact- en adresgegevens zijn opgeslagen. Vul je bankrekening- en belastinginformatie in als je dit nog niet hebt gedaan.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Fout', 'Er is een onbekende fout opgetreden bij het opslaan van je contactgegevens.' );
					}
				}
			} );
		}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Fout', 'Het opgegeven profiel kan niet worden gevonden.' );
						break;
				}
			}
		}(publishedfileid))
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Even geduld', 'Opties worden geladen ...' );
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		switch ( rgResults['success'] )
		{
			case 14:
				...
				break;
			default:
				ShowAlertDialog( 'Fout', 'Er is een fout opgetreden bij het verwerken van je verzoek.' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
