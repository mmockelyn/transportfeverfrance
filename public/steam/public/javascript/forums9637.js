...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = '如果您的库存隐私设置为“公开”，社区成员可以在该主题中将交易报价发送给您。';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>您的库存隐私已被设置为“公开”</b>。这表示社区成员可以在该主题中将交易报价发送给您。如果您的库存隐私并未设置为“公开”，您将无法收到社区交易报价。';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>您的库存隐私<u>并未</u>设置为“公开”</b>。这表示社区成员无法在该主题中将交易报价发送给您。如果您的库存隐私设置为“公开”，社区成员可以在该主题中将交易报价发送给您。';
			...
			strMessage += '一旦您收到交易报价或完成交易，您可以在此关闭该主题并禁止交易报价。';
...
{
	ShowConfirmDialog( '关闭该主题', '将该主题标记为“已关闭”将阻止任何的回复和交易报价。一旦它被关闭，您将无法对其采取任何操作。', '关闭该主题')
	.done( function() {
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( '标记为垃圾信息', '已删除选中主题并封禁创建者。' );
		});
});
}
