	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Nội dung có thể không phù hợp cho mọi lứa tuổi', data['html'], 'Xem nội dung' );
				...
				{
					ShowAlertDialog('Nội dung có thể không phù hợp cho mọi lứa tuổi', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Xem cảnh báo nội dung người lớn' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Bỏ mờ ảnh chỉ cho nội dung này' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Làm mờ nội dung này' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Nội dung có thể không phù hợp với mọi khán giả'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Đánh dấu spam', 'Việc này sẽ áp lệnh cấm cộng đồng lên tác giả, thêm bình luận này như dữ liệu đào tạo để chúng tôi có thể đánh dấu những loại bình luận này một cách tự động, xóa bỏ mọi bình luận của tác giả. Bạn có chắc đây là điều mình muốn làm?', 'Đánh dấu spam' );
