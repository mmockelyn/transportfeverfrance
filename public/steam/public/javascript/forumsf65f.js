...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Als je inventarisprivacy openbaar is, zullen communityleden je via dit onderwerp ruilaanbiedingen kunnen sturen.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>Je inventarisprivacy is Openbaar</b>. Dit betekent dat communityleden je ruilaanbiedingen via dit onderwerp kunnen sturen. Als je inventarisprivacy op niet Openbaar zou staan, zouden communityleden je geen ruilaanbiedingen kunnen sturen.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>Je inventarisprivacy is <u>niet</u> Openbaar</b>. Dit betekent dat communityleden je geen ruilaanbiedingen via dit onderwerp kunnen sturen. Als je inventarisprivacy op Openbaar zou staan, zouden communityleden je wél ruilaanbiedingen via dit onderwerp kunnen sturen.';
			...
			strMessage += 'Wanneer je klaar bent met ruilaanbiedingen ontvangen of de ruil voltooid hebt, kun je dit onderwerp sluiten en ruilaanbiedingen die vanaf hier gemaakt worden stoppen.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Het maken van het topic is mislukt:' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Op forum abonneren', 'Er was een probleem met het abonneren op dit forum.', 'Probeer het later opnieuw.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Uitschrijven van forum', 'Er was een probleem met het opzeggen van het abonnement op dit forum.', 'Probeer het later opnieuw.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Topic verwijderen ',
				'Weet je zeker dat je dit topic wilt verwijderen? ' +
				'Deze actie kan alleen door een moderator ongedaan worden gemaakt.',
				'Topic verwijderen'
			).done( function() {
					Topic.Delete();
				});
...
{
	ShowConfirmDialog('Permanent verwijderen ',
		'Weet je zeker dat je dit onderwerp voorgoed wilt verwijderen? Dit kan niet ongedaan worden gemaakt.',
...
{
	ShowConfirmDialog('Markering als verboden inhoud opheffen ',
		'Weet je zeker dat je de markering \'verboden inhoud\' voor dit onderwerp wilt wissen? Dit kan niet ongedaan worden gemaakt.',
		'Markering als verboden inhoud opheffen'
	).done( function() {
...
{
	ShowConfirmDialog('Reactiegevers verbannen ',
	'Weet je zeker dat je gebruikers die meer dan 5 maal een reactie hebben geplaatst in dit onderwerp wilt verbannen? Het is moeilijk om dit ongedaan te maken.',
	'Reactiegevers verbannen'
).done( function() {
...
{
	ShowConfirmDialog('Markeren als spam ',
		'Dit verwijdert de geselecteerde discussies en verbant de auteurs van het forum. Weet je zeker dat je dit wilt doen?',
		'Spam verwijderen'
	).done( function() {
			...
			.done( function( transport ) {
				...
				if ( strNewURL )
				{
					ShowConfirmDialog(
						...
						'De geselecteerde topics zijn samengevoegd in één topic.',
						'Naar het samengevoegde onderwerp gaan',
...
{
	ShowConfirmDialog( 'Dit onderwerp sluiten', 'Als je dit onderwerp als "Gesloten" markeert zul je voorkomen dat gebruikers berichten kunnen plaatsen of ruilaanbiedingen kunnen versturen. Zodra het gesloten is, zul je geen andere handelingen erop kunnen toepassen.', 'Dit onderwerp sluiten')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Rapporteren', 'Bedankt voor je rapport', 'Je melding is ter beoordeling naar de moderators gestuurd. We laten het je weten als er actie wordt ondernomen.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Onderwerp wijzigen mislukt. Foutmelding:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Aanpassen van topic mislukt. Probeer het later opnieuw.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Abonneren op discussie', 'Je bent geabonneerd op deze discussie.', 'Je ontvangt een melding van een nieuwe reactie als iemand op deze discussie reageert.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Abonneren op discussie', 'Sorry, het abonneren op de discussie is mislukt. Probeer het later opnieuw.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Abonnement op discussie opzeggen', 'Je abonnement op deze discussie is opgezegd.', 'Je ontvangt geen melding van nieuwe reacties op deze discussie meer.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Abonnement op discussie opzeggen', 'Sorry, het uitschrijven van deze discussie is mislukt. Probeer het later opnieuw.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Alle meldingen in verband met dit onderwerp zijn als opgelost gemarkeerd.<br>Aantal meldingen dat moest opgelost worden:' + cReportsResolved;
				else
					strMessage = 'Alle meldingen in verband met dit onderwerp zijn als opgelost gemarkeerd.';

				ShowForumSuccessDialogWithDetailTitle( 'Gerapporteerde berichten oplossen', 'Rapporten afgehandeld.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Berichtgeschiedenis', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Weet je zeker dat je dit bericht wilt verwijderen?', 'Verwijderen' )
...
var g_rgRedirectTimeOptions = [
	{value: 0, label: 'Geen verwijzing' },
	{value: 1, label: '1 uur' },
	{value: 24, label: '1 dag' },
	{value: 168, label: '1 week' },
	{value: 720, label: '1 maand' },
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'Het samenvoegen van onderwerpen zal alle berichten van één of meerdere onderwerpen verplaatsen naar een ander onderwerp. Indien er al berichten in het aangeduide onderwerp zijn, zullen nieuwe berichten elkaar chronologisch opvolgen. Dit kan niet ongedaan worden gemaakt.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Selecteer een recent bekeken onderwerp om deze onderwerpen mee samen te voegen:' : 'Selecteer een recent bekeken onderwerp om dit onderwerp mee samen te voegen:';
	...
	var strMoreInstructions = 'Als het onderwerp dat je wilt samenvoegen niet in deze lijst staat, dan kun je <%1$s>zoeken naar het onderwerp<%2$s> in een nieuw venster. Deze lijst zal automatisch geüpdatet worden wanneer je hier terugkomt.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Verwijzing achterlaten: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Voeg de geselecteerde onderwerpen samen in het oudste onderwerp' );
	...
	var Modal = ShowConfirmDialog( 'Onderwerpen samenvoegen', $DialogContent, 'Onderwerpen samenvoegen' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Verwijderen ongedaan maken', 'De geselecteerde topics zijn hersteld.' );
			else
				ShowAlertDialog( 'Verwijderen', 'De geselecteerde topics zijn verwijderd.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Ontgrendelen', 'De geselecteerde topics zijn ontgrendeld.' );
			else
				ShowAlertDialog( 'Sluiten', 'De geselecteerde topics zijn gesloten.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Verplaatsen ...', 'De geselecteerde topics zijn verplaatst naar het nieuwe forum.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Onderwerpen samenvoegen',
					'De geselecteerde topics zijn samengevoegd in één topic.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Permanent verwijderen ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Permanent verwijderen', 'De geselecteerde onderwerpen zijn permanent verwijderd.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Markeren als spam', 'De geselecteerde onderwerpen zijn verwijderd en de makers zijn verbannen.' );
		});
});
}
...
{
	ShowConfirmDialog('Reactiegevers verbannen ',
	'Dit stelt een communityverbanning in jegens (of markeert als \'gekaapt\') gebruikers die in de geselecteerde onderwerpen meer dan een gegeven aantal maal reacties hebben geplaatst. Weet je zeker dat je dit wilt doen?',
	'Reactiegevers verbannen'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Reactiegevers verbannen', 'De gebruikers die meer reacties hebben geplaatst bij de geselecteerde onderwerpen dan de drempelwaarde, zijn verbannen.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Fout', 'Het verwerken van het geheel of een deel van je verzoek is mislukt. Verifieer dat je de verwachte resultaten hebt.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Gebruiker waarschuwen', $WaitElem, 'Gebruiker waarschuwen' );
	} else {
		var Modal = ShowConfirmDialog( 'Gebruiker bannen', $WaitElem, 'Gebruiker bannen' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Gebruiker bannen', 'Er moet minimaal één optie worden geselecteerd. De gebruiker is niet verbannen.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Gebruiker bannen', ban_response.message ? ban_response.message : 'De rechten om berichten te plaatsen en te bewerken zijn voor deze gebruiker ingetrokken.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Gebruiker bannen', ban_response.message ? ban_response.message : 'Mogelijk heb je geen toestemming om gebruikers te verbannen. Controleer de rechten van je account of probeer het later opnieuw.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Gebruiker waarschuwen', ban_response.message ? ban_response.message : 'Deze gebruiker is met succes gewaarschuwd voor zijn of haar bericht.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Gebruiker waarschuwen', ban_response.message ? ban_response.message : 'We zijn op een probleem gestuit bij het waarschuwen van deze gebruiker. Probeer het later opnieuw.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Gebruiker bannen', 'Mogelijk heb je geen toestemming om gebruikers te verbannen. Controleer de rechten van je account of probeer het later opnieuw.' );
		}
		else {
			ShowAlertDialog( 'Gebruiker waarschuwen', 'We zijn op een probleem gestuit bij het waarschuwen van deze gebruiker. Probeer het later opnieuw.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Voer het bericht in dat je privé naar deze gebruiker wilt versturen:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">* Dit bericht wordt automatisch in je privébericht vermeld.</div>';

	var $modal = ShowConfirmDialog( 'Privébericht', $strDialogHTML, 'Privébericht versturen' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Privébericht', response.message );
				}
				else
				{
					ShowAlertDialog( 'Fout', 'Er is een fout opgetreden bij het verzenden van dit privébericht. Probeer het opnieuw.' + '(' + response.success + ')' );
				}
			});
	} );
