...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Data zamieszczenia' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Od' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'do' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Data ostatniej aktualizacji'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Od' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'do' } ) );
	...
	var dialog = ShowConfirmDialog( 'Filtruj według daty', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Nazwa firmy';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Imię (jak podano w danych bankowych)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Numer rozliczeniowy';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Kanadyjski Numer Tranzytowy';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Wprowadź nazwę firmy.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Podaj poprawne imię.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Podaj poprawne nazwisko.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Podaj poprawny adres.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Podaj poprawną miejscowość.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Podaj poprawny kod pocztowy.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Podaj numer telefonu razem z kodem kraju.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Zapisano!', response.redirect_url.length != 0 ? 'Twoje dane kontaktowe oraz adres zostały zapisane. Z powodu zmiany kraju przekierujemy cię teraz do zewnętrznego serwisu w celu aktualizacji informacji podatkowych.' : 'Twoje dane kontaktowe oraz adresowe zostały zapisane. Podaj swoje informacje bankowe i podatkowe w przypadku ich braku.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Błąd', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Błąd', 'Wystąpił nieznany błąd przy próbie zapisu twoich danych kontaktowych.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Podaj poprawny adres e-mail.<br/>';
		}
		...
		if ( $( 'bankcountry' ).value == 'US' )
		{
			...
			{
				errorString += 'Podaj swój numer rozliczeniowy (9 cyfr).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Wprowadź Kanadyjski Numer Tranzytowy (9 cyfr z zerem na początku).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Podaj swój kod IBAN.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Wprowadź kod SWIFT swojego banku.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Zarządzaj współautorami', 'Wczytywanie listy znajomych' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Zarządzaj współautorami', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Anuluj' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Błąd', 'Nie można odnaleźć określonego profilu.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Czy na pewno chcesz zamknąć listę twórców? Po tym kroku nie będziesz mieć możliwości ich usuwania lub dodawania.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Proszę czekać', 'Wczytywanie opcji...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Wybierz dostarczycieli usługi';
		var strSaveChanges = 'Kontynuuj';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Błąd', 'Podział przychodu dla dostawców usług musi sumarycznie wynosić 100%.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Błąd', 'Nie możesz wybrać organizacji, która jest współtwórcą twojego przedmiotu.' );
				break;
			default:
				ShowAlertDialog( 'Błąd', 'Wystąpił błąd przetwarzania żądania użytkownika:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
