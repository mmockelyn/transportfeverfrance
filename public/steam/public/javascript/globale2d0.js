	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Deze inhoud is mogelijk niet geschikt voor alle leeftijden', data['html'], 'Inhoud weergeven' );
				...
				{
					ShowAlertDialog('Deze inhoud is mogelijk niet geschikt voor alle leeftijden', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Waarschuwing over inhoud voor volwassenen bekijken' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Alleen van deze inhoud de vervaging verwijderen' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Deze inhoud vervagen' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Deze inhoud is mogelijk niet geschikt voor alle leeftijden'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Markeren als spam', 'Hierdoor wordt de auteur ervan verbannen uit de Community, wordt deze reactie toegevoegd aan de trainingsgegevens voor ons algoritme zodat we dit type reacties in de toekomst automatisch kunnen markeren, en worden alle reacties van deze auteur gewist. Weet je zeker dat je dit wilt doen?', 'Markeren als spam' );
