...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Îți mulțumim!', 'Îţi mulţumim că ne-ai raportat conţinutul ofensator şi ne ajuţi să menţinem comunitatea Steam curată şi prietenoasă.' );
	}).fail( function() {
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Notă: URL-ul pe care ai dat click nu este un site oficial Steam.\n\n'
						...
						+ 'Dacă acest site web îți cere numele de utilizator sau parola, nu introdu aceste informații. Ai putea să-ţi pierzi contul Steam şi toate jocurile!\n'
	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Conținutul poate să nu fie adecvat pentru toate vârstele', data['html'], 'Vezi conținutul' );
				...
				{
					ShowAlertDialog('Conținutul poate să nu fie adecvat pentru toate vârstele', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Vezi avertismentul pentru conținutul destinat adulților' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Elimină cenzura doar pentru acest conținut' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Cenzurează acest conținut' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Conținutul poate să nu fie adecvat pentru toate segmentele de public'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Marchează ca spam', 'Autorul va primi o interdicție de a folosi comunitatea, iar toate comentariile lui vor fi șterse. Acest comentariu va fi folosit ca referință pentru a putea marca astfel de comentarii în mod automat. Ești sigur ca dorești să continui?', 'Marchează ca spam' );
