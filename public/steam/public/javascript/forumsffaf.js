...
var CForum = Class.create( {
	...
	UpdateCreateTopicFormDisplay: function()
	{
		...
		{
			var strMessage = 'Se il tuo inventario è impostato come Pubblico, i membri della Comunità potranno inviarti offerte di scambio da questa discussione.';
			if ( this.m_rgCreateTopicFlags.inventory_public )
				strMessage = '<b>Il tuo inventario è attualmente impostato come Pubblico</b>. Ciò significa che i membri della Comunità possono inviarti offerte di scambio da questa discussione. Se il tuo inventario utilizzerà un livello di privacy diverso da quello Pubblico, non potrai ricevere offerte di scambio dalla Comunità.';
			else if ( typeof this.m_rgCreateTopicFlags.inventory_public != 'undefined' )
				strMessage = '<b>Il tuo inventario <u>non</u> è impostato come Pubblico</b>. Ciò significa che i membri della Comunità non potranno inviarti offerte di scambio da questa discussione. Impostare il tuo inventario come Pubblico permetterà ai membri della Comunità di inviarti offerte di scambio da questa discussione.';
			...
			strMessage += 'Quando non vuoi più ricevere le offerte di scambio o hai completato lo scambio, puoi chiudere la discussione e disattivare le offerte di scambio da essa provenienti da qui.';
	...
	OnNewTopicFailure: function( transport )
	{
		this.m_bSubmittingTopic = false;
		var rgJSON = transport.responseJSON || {};
		$('forum_' + this.m_strName + '_newtopic_error' ).update( 'Si è verificato un errore durante la creazione di una nuova discussione:' + ( rgJSON.error ? rgJSON.error : rgJSON.success ) );
	...
	SubscribeToForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'subscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Iscriviti alla sezione', 'Si è verificato un errore durante l\'iscrizione a questa sezione.', 'Riprova più tardi.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	},
	...
	UnsubscribeFromForum: function()
	{
		...
		new Ajax.Request( this.GetActionURL( 'unsubscribe' ), {
			...
			onFailure: function() { ShowForumSuccessDialogWithDetailTitle( 'Annulla l\'iscrizione alla sezione', 'Si è verificato un errore durante l\'annullamento dell\'iscrizione a questa sezione.', 'Riprova più tardi.' ); },
			onComplete: this.OnAJAXComplete.bind( this )
		});
	}
...
function Forum_DeleteTopic( gidTopic )
{
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		{
			ShowConfirmDialog('Elimina la discussione ',
				'Sei sicuro di voler eliminare questa discussione? ' +
				'Questa azione può essere annullata solo da un moderatore.',
				'Elimina la discussione'
			).done( function() {
					Topic.Delete();
				});
		}
...
{
	ShowConfirmDialog('Elimina definitivamente ',
		'Sei sicuro di voler eliminare definitivamente questa discussione? Questa azione non può essere annullata.',
...
{
	ShowConfirmDialog('Rimuovi il contrassegno di contenuto passibile di ban ',
		'Sei sicuro di voler rimuovere il contrassegno di contenuto passibile di ban? Questa azione è irreversibile.',
		'Rimuovi il contrassegno di contenuto passibile di ban'
	).done( function() {
...
{
	ShowConfirmDialog('Ban dei commentatori ',
	'Sei sicuro di voler bandire gli utenti che hanno commentato più di 5 volte in questa discussione? Sarà difficile annullare questa azione.',
	'Ban dei commentatori'
).done( function() {
...
{
	ShowConfirmDialog('Contrassegna come spam ',
		'In questo modo eliminerai le discussioni selezionate e bannerai gli autori dal forum. Confermi?',
		'Elimina spam'
	).done( function() {
	...
	if ( g_rgForumTopics[ gidTopic ] )
	{
		...
		Forum_MergeTopicDialog( Topic.GetActionURL( 'mergetopics' ), Topic.m_rgForumData, [ gidTopic ] )
			.done( function( transport ) {
				...
				{
					ShowConfirmDialog(
						'Unisci le discussioni',
						'Le discussioni selezionate sono state unite.',
						'Vai alla discussione unita',
						'Ritorna alla lista delle discussioni'
					).done( function() {
...
{
	ShowConfirmDialog( 'Chiudi questa discussione', 'Indicare questa discussione come "Chiusa" non permetterà la creazione di nuove risposte o offerte di scambio al suo interno. Una volta che avrai chiuso questa discussione, non potrai eseguirci ulteriori azioni.', 'Chiudi questa discussione')
	.done( function() {
...
CForumTopic = Class.create( {
	...
	{
		ShowForumSuccessDialogWithDetailTitle( 'Segnala', 'Grazie per la tua segnalazione', 'La tua segnalazione è stata inviata ai moderatori per la revisione. Ti faremo sapere qualora verranno presi dei provvedimenti.' );
	},
	...
	OnModeratorActionFailed: function( transport )
	{
		...
		{
			ShowForumSuccessDialog( '', 'Impossibile modificare la discussione. Messaggio d\'errore:' +
			 "<br><br>" + transport.responseJSON.msg );
		}
		else
		{
			ShowForumSuccessDialog( '', 'Modifica della discussione non riuscita. Riprova più tardi.' );
		}
	},
	...
	Subscribe: function( fnExternalOnSuccess )
	{
		if ( g_rgForumTopicCommentThreads[ this.m_gidForumTopic ] )
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Iscriviti alla discussione', 'Ti sei iscritto a questa discussione.', 'Verrai notificato dei nuovi commenti in questa discussione.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Iscriviti alla discussione', 'Siamo spiacenti, ma è stato impossibile eseguire l\'iscrizione a questa discussione. Riprova più tardi.');
	...
	Unsubscribe: function( fnExternalOnSuccess )
	{
		...
		{
			var fnSuccess = function() {
				ShowForumSuccessDialogWithDetailTitle( 'Annulla l\'iscrizione alla discussione', 'L\'iscrizione a questa discussione è stata rimossa.', 'Non riceverai più notifiche sui nuovi commenti in questa discussione.' );
				fnExternalOnSuccess();
			};
			var fnFail = ShowForumSuccessDialog.bind( null, 'Annulla l\'iscrizione alla discussione', 'Siamo spiacenti, ma è stato impossibile annullare l\'iscrizione a questa discussione. Riprova più tardi.');
		...
		new Ajax.Request( this.GetActionURL( 'moderatetopic' ), {
			...
			onSuccess: function(transport) {
				...
				if ( cReportsResolved > 0 )
					strMessage = 'Tutte le segnalazioni associate a questa discussione sono state contrassegnate come risolte.<br>Numero di segnalazioni che dovevano essere risolte: ' + cReportsResolved;
				else
					strMessage = 'Tutte le segnalazioni associate a questa discussione sono già state contrassegnate come risolte.';

				ShowForumSuccessDialogWithDetailTitle( 'Risolvi i messaggi segnalati', 'Segnalazioni risolte.', strMessage );
	...
	ShowAudits: function ( gidObject )
	{
		...
		$J.get( strURL, rgParams ).done( function(data) {
			ShowAlertDialog( 'Cronologia del messaggio', data );
		});
	},
...
CCommentThreadForumTopic = Class.create( CCommentThread, {
	...
	DeleteComment: function( $super, gidComment, bUndelete, fnOnSuccess )
	{
		...
		{
			ShowConfirmDialog( '', 'Sei sicuro di voler eliminare questo messaggio?', 'Elimina' )
...
var g_rgRedirectTimeOptions = [
	{value: 0, label: 'Nessun reindirizzamento' },
	{value: 1, label: '1 ora' },
...
function Forum_MergeTopicDialog( strActionURL, rgForumData, rgForumTopics, bResolveReports )
{
	...
	var strMessage = 'Unendo queste discussioni, sposterai tutti i messaggi di una o più discussioni in un\'altra discussione. Se esistono già dei messaggi nella discussione scelta, quelli nuovi verranno interposti in ordine cronologico. Questa azione non può essere annullata.<br><br>';
	strMessage += rgForumTopics.length > 1 ? 'Seleziona una discussione vista di recente per unirvi queste:' : 'Seleziona una discussione vista di recente per unirvi questa:';
	...
	var strMoreInstructions = 'Se la discussione che vuoi unire non è in questa lista, puoi <%1$s>cercare quella desiderata<%2$s> in una nuova finestra. La lista verrà aggiornata automaticamente al tuo ritorno.'.replace( /%1\$s/, 'a class="whiteLink" href="' + rgForumData.forum_url + '" target="_blank"').replace( /%2\$s/, '/a' );
	...
	$Expiry.append( 'Lascia reindirizzamento per: ', $Select );
	...
	var fnPopulateMergeOptions = function() {
		...
		{
			var $MergeTogether = fnMakeMergeOption( 0, 0, 0, 'Unisci le discussioni selezionate con quella più vecchia.' );
	...
	var Modal = ShowConfirmDialog( 'Unisci le discussioni', $DialogContent, 'Unisci le discussioni' );
...
function ForumBulkDelete( strName, rgForumTopicGIDs, bUndelete )
{
	BulkModerate( strName, rgForumTopicGIDs, 'deleted', !bUndelete)
		.done( function( data ) {
			if ( bUndelete )
				ShowAlertDialog( 'Ripristina', 'Le discussioni selezionate sono state ripristinate.' );
			else
				ShowAlertDialog( 'Elimina', 'Le discussioni selezionate sono state eliminate.' );
		});
}
...
function ForumBulkLock( strName, rgForumTopicGIDs, bUnlock )
{
	BulkModerate( strName, rgForumTopicGIDs, 'locked', !bUnlock)
		.done( function( data ) {
			if ( bUnlock )
				ShowAlertDialog( 'Riapri', 'Le discussioni selezionate sono state riaperte.' );
			else
				ShowAlertDialog( 'Chiudi', 'Le discussioni selezionate sono state chiuse.' );
		});
}
...
function ForumBulkMove( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'movetopic' )
		.done( function( transport ) {
			// the transport is from prototype
			ShowAlertDialog( 'Sposta...', 'Le discussioni selezionate sono state spostate nella nuova sezione.' );
		});
}
...
function ForumBulkMerge( strName, rgForumTopicGIDs )
{
	return BulkModerate( strName, rgForumTopicGIDs, 'mergetopics' )
		.done( function( transport ) {
			...
			{
				// don't have a URL?
				ShowAlertDialog('Unisci le discussioni',
					'Le discussioni selezionate sono state unite.' );
			}
		} );
}
...
function ForumBulkPurge( strName, rgForumTopicGIDs, fnOnComplete )
{
	ShowConfirmDialog('Elimina definitivamente ',
		...
		.done( function( data ) {
			ShowAlertDialog( 'Elimina definitivamente', 'Le discussioni selezionate sono state eliminate definitivamente.' );
			fnOnComplete();
		});
	});
}
...
function ForumBulkSpam( strName, rgForumTopicGIDs )
{
	...
	BulkModerate( strName, rgForumTopicGIDs, 'markspam', true )
		.done( function( data ) {
			ShowAlertDialog( 'Contrassegna come spam', 'Le discussioni selezionate sono state eliminate e i relativi creatori banditi.' );
		});
});
}
...
{
	ShowConfirmDialog('Ban dei commentatori ',
	'In questo modo applicherai un ban della Comunità (o contrassegnerai come fraudolenti) gli utenti che hanno commentato nelle discussioni selezionate più di un certo numero di volte. Confermi?',
	'Ban dei commentatori'
).done ( function() {
	BulkModerate( strName, rgForumTopicGIDs, 'bancommenters', true )
		.done( function( data ) {
			ShowAlertDialog( 'Ban dei commentatori', 'Gli utenti che hanno commentato un numero di volte superiore alla soglia permessa nelle discussioni selezionate sono stati banditi.' );
		});
});
}
...
function BulkModerate( strName, rgForumTopicGIDs, flag, value )
{
	...
	return deferred.fail( function() {
		ShowAlertDialog( 'Errore', 'Si è verificato un errore durante l\'elaborazione di tutta o parte della tua richiesta. Assicurati di aver ottenuto i risultati voluti.' );
	}).always( function() {
...
function Forum_BanOrWarnUser( clanid, gidForum, gidTopic, gidComment, accountIDTarget, bWarning )
{
	...
	if ( bWarning ) {
		var Modal = ShowConfirmDialog( 'Avvisa utente', $WaitElem, 'Avvisa utente' );
	} else {
		var Modal = ShowConfirmDialog( 'Bandisci l\'utente', $WaitElem, 'Bandisci l\'utente' );
	}
	...
	$J.get( 'https://steamcommunity.com/gid/' + clanid + '/banwarnuserdialog', {
		...
		if ( !bWarning )
		{
			...
			Modal.done( function() {
				...
				{
					ShowAlertDialog( 'Bandisci l\'utente', 'Dev\'essere selezionata almeno un\'opzione. L\'utente non è stato bandito.' );
				}
				else
				{
					$J.post(
						...
						ShowAlertDialog( 'Bandisci l\'utente', ban_response.message ? ban_response.message : 'I privilegi di pubblicazione e modifica dell\'utente sono stati revocati.' );
					}).fail( function( jqxhr ) {
						var ban_response = V_ParseJSON( jqxhr.responseText );
						ShowAlertDialog( 'Bandisci l\'utente', ban_response.message ? ban_response.message : 'Potresti non avere i permessi per bandire altri utenti. Controlla i permessi del tuo account o riprova più tardi.' );
					});
				}
			});
		}
		else
		{
			Modal.done( function() {
				...
				$J.post(
					...
					ShowAlertDialog( 'Avvisa utente', ban_response.message ? ban_response.message : 'L\'utente è stato avvisato con successo per il suo messaggio.' );
				}).fail( function( jqxhr ) {
					var ban_response = V_ParseJSON( jqxhr.responseText );
					ShowAlertDialog( 'Avvisa utente', ban_response.message ? ban_response.message : 'Si è verificato un errore durante l\'invio di questo avviso all\'utente in questione. Riprova più tardi.' );
				});
			});
		}
		...
		Modal.Dismiss();
		if ( !bWarning ) {
			ShowAlertDialog( 'Bandisci l\'utente', 'Potresti non avere i permessi per bandire altri utenti. Controlla i permessi del tuo account o riprova più tardi.' );
		}
		else {
			ShowAlertDialog( 'Avvisa utente', 'Si è verificato un errore durante l\'invio di questo avviso all\'utente in questione. Riprova più tardi.' );
		}
	});
}
...
{
	var $strDialogHTML = '<div class="private_message_dialog_instructions" >' + 'Inserisci il messaggio che vorresti inviare privatamente a questo utente:' + '</div><div class="fullwidth gray_bevel"><textarea class="forumtopic_reply_textarea" rows="3" name="pm_text" id="pm_text" ></textarea></div><div class="private_message_dialog_footnote">*Nel tuo messaggio privato si farà automaticamente riferimento a questo messaggio.</div>';

	var $modal = ShowConfirmDialog( 'Messaggio privato', $strDialogHTML, 'Invia messaggio privato' );
	$modal.GetContent().css('width', '600px');
	...
	$modal.done( function( ) {
		...
		$J.post('https://steamcommunity.com/gid/' + clanid + '/sendprivatemessage/', rgParams )
			.done( function ( response ) {
				...
				{
					ShowAlertDialog( 'Messaggio privato', response.message );
				}
				else
				{
					ShowAlertDialog( 'Errore', 'Si è verificato un errore durante l\'invio di questo messaggio privato. Riprova.' + '(' + response.success + ')' );
				}
			});
	} );
