...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Datum přidání' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Mezi' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'A' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Datum poslední aktualizace'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Mezi' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'A' } ) );
	...
	var dialog = ShowConfirmDialog( 'Filtrovat dle data', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Název společnosti';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Křestní jméno (jak je uvedeno v bankovních záznamech)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Směrovací číslo (pouze USA)';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Kanadské tranzitní číslo';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Zadejte název Vaší společnosti.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Zadejte platné křestní jméno.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Zadejte platné příjmení.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Zadejte platnou adresu.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Zadejte platné město.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Zadejte platné PSČ.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Zadejte prosím vaše telefonní číslo (včetně mezinárodní předvolby).<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Uloženo!', response.redirect_url.length != 0 ? 'Vaše kontaktní údaje byly uloženy. Jelikož jste aktualizovali svoji zemi, budete nyní přesměrováni na externí stránku, kde budete muset aktualizovat své daňové informace.' : 'Vaše kontaktní údaje byly uloženy. Pokud jste tak ještě neučinili, vyplňte i údaje o svém bankovním účtu a daňové informace.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Chyba', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Chyba', 'Při ukládání Vašich kontaktních údajů došlo k neznámé chybě.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Zadejte platnou e-mailovou adresu.<br/>';
		}
		...
		if ( $( 'bankcountry' ).value == 'US' )
		{
			...
			{
				errorString += 'Zadejte směrovací číslo své banky (9 číslic).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Zadejte své kanadské tranzitní číslo (9 čísel i s 0 na začátku).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Zadejte své číslo IBAN.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Zadejte kód SWIFT své banky.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Spravovat přispěvatele', 'Načítání seznamu přátel' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Spravovat přispěvatele', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Zrušit' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Chyba', 'Zadaný profil nebyl nalezen.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Opravdu chcete finalizovat přispěvatele? Po tomto kroku je již nebudete moci přidat ani odstranit.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Čekejte prosím', 'Načítání možností...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Vyberte poskytovatele služeb';
		var strSaveChanges = 'Pokračovat';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Chyba', 'Součet procentuálních podílů přidělených poskytovatelům služeb se v součtu musí rovnat 100.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Chyba', 'Nemůžete vybrat organizaci třetí strany, která je u Vaší položky zároveň i přispěvatelem.' );
				break;
			default:
				ShowAlertDialog( 'Chyba', 'Při zpracovávání Vašeho požadavku došlo k chybě:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
