	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Obsah nemusí být vhodný pro některé věkové kategorie', data['html'], 'Zobrazit obsah' );
				...
				{
					ShowAlertDialog('Obsah nemusí být vhodný pro některé věkové kategorie', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Zobrazit upozornění na obsah pro dospělé' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Zakázat rozostření pouze u tohoto obsahu' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Rozostřit tento obsah' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Obsah nemusí být vhodný pro některé věkové kategorie'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Označit jako spam', 'Touto akcí uvalíte na autora ban v komunitě, přidáte tento komentář mezi data, která nám pomohou rozeznávat podobné komentáře automaticky, a také smažete všechny zbylé komentáře tohoto autora. Opravdu chcete pokračovat?', 'Označit jako spam' );
