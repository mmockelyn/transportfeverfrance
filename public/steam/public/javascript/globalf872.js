	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Inhalte möglicherweise nicht für alle Altersgruppen geeignet', data['html'], 'Inhalt anzeigen' );
				...
				{
					ShowAlertDialog('Inhalte möglicherweise nicht für alle Altersgruppen geeignet', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Warnung für nicht jugendfreie Inhalte anzeigen' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Nur diesen Inhalt nicht mehr verpixeln' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Diesen Inhalt verpixeln' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Inhalte möglicherweise nicht für alle Zielgruppen geeignet'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Als Spam markieren', 'Hierdurch wird der Ersteller aus der Community ausgeschlossen und alle Kommentare des Erstellers werden gelöscht. Die Daten dieses Kommentars werden zu Trainingszwecken analysiert, um solche Kommentare automatisch markieren zu können. Möchten Sie wirklich fortfahren?', 'Als Spam markieren' );
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Kommentare deabonnieren', 'Sie erhalten keine Benachrichtigungen mehr über neue Kommentare.');
			}, function() {
				ShowAlertDialog('Kommentare deabonnieren', 'Beim Aktualisieren Ihres Abonnements ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut.');
			});
		});
