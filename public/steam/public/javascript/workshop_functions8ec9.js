...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Дата публікації' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'З' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'до' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Дата останнього оновлення'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'З' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'до' } ) );
	...
	var dialog = ShowConfirmDialog( 'Фільтрувати за датою', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Назва компанії';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Ім’я (згідно з банківським записом)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Код банку США (Routing Number)';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Код канадського банку (Canadian Transit Number)';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Будь ласка, введіть назву компанії.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Будь ласка, введіть своє дійсне ім’я.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Будь ласка, введіть своє дійсне прізвище.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Будь ласка, введіть свою дійсну адресу.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Будь ласка, введіть дійсну назву вашого населеного пункту.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Будь ласка, введіть свій дійсний поштовий індекс або Zip-код.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Будь ласка, введіть свій номер телефону, з кодом міста включно.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Збережено!', response.redirect_url.length != 0 ? 'Вашу контактну інформацію та адресу було збережено. Оскільки ви оновили свою країну, вас буде перенаправлено до зовнішнього сервісу для оновлення податкової інформації.' : 'Вашу контактну інформацію було збережено. Якщо ви ще не заповнили банківську та податкову інформацію, то зробіть це, будь ласка.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Помилка', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Помилка', 'Сталася невідома помилка при спробі збереження вашої контактної інформації.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Будь ласка, введіть вашу дійсну адресу електронної пошти.<br/>';
		}
		...
		{
			errorString += 'Будь ласка, введіть дійсну назву вашого банку.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Будь ласка, введіть дійсне повне ім’я власника банківського рахунка (повинне збігатися з документацією).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Будь ласка, введіть дійсну адресу вашого банку.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Будь ласка, введіть дійсне місто розташування вашого банку.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Будь ласка, вкажіть дійсний поштовий індекс або ZIP-код вашого банку.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Будь ласка, введіть номер вашого банківського рахунка.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Будь ласка, введіть код вашого банку в системі обліку США (9 цифр).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Будь ласка, введіть код вашого банку в системі обліку Канади (9 цифр включно з 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Будь ласка, введіть код IBAN вашого банку.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Будь ласка, введіть код SWIFT вашого банку.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Керувати співавторами', 'Завантаження списку друзів' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Керувати співавторами', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Скасувати' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Помилка', 'Вказаний профіль не було знайдено.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Ви впевнені, що хочете затвердити цих співавторів? Опісля ви вже не зможете додавати або вилучати їх.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Будь ласка, зачекайте', 'Триває завантаження…' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Оберіть постачальників послуг';
		var strSaveChanges = 'Продовжити';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Помилка', 'Винагорода постачальника послуг не повинна перевищувати 100%.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Помилка', 'Ви не можете вибрати співавтора вашого продукту в якості сторонньої організації.' );
				break;
			default:
				ShowAlertDialog( 'Помилка', 'Під час обробки вашого запиту сталася помилка:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
