	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Treści mogą nie być odpowiednie dla wszystkich grup wiekowych', data['html'], 'Pokaż treści' );
				...
				{
					ShowAlertDialog('Treści mogą nie być odpowiednie dla wszystkich grup wiekowych', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Wyświetl ostrzeżenie przed treściami dla dorosłych' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Usuń rozmycie wyłącznie dla tej treści' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Zastosuj rozmycie dla tej treści' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Treści mogą nie być odpowiednie dla wszystkich odbiorców'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Oznacz spam', 'To działanie sprawi, że autor otrzyma blokadę społeczności, doda ten komentarz do naszych danych uczących, byśmy mogli oznaczać tego typu komentarze automatycznie, a także wyczyści wszystkie komentarze autora. Czy na pewno chcesz to zrobić?', 'Oznacz spam' );
