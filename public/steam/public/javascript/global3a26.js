	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Вміст може бути непридатний для усіх вікових груп', data['html'], 'Переглянути вміст' );
				...
				{
					ShowAlertDialog('Вміст може бути непридатний для усіх вікових груп', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Переглянути попередження про вміст для дорослих' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Вилучити розмиття лише для цього вмісту' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Розмити цей вміст' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Вміст може бути неприйнятним для всієї аудиторії'} ) );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Позначити як спам', 'Автор отримає блокування спільноти, а всі його коментарі будуть видалені. Цей коментар буде використано в навчальних цілях для автоматичного позначення коментарів такого типу. Ви дійсно бажаєте зробити це?', 'Позначити як спам' );
	...
	ClearContentCheckFlag: function( gidComment, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Зняти мітку про заблокований вміст', 'Мітку про заблокований вміст буде знято з цього коментаря. Ця дія незворотна. Ви дійсно бажаєте це зробити?', 'Зняти мітку про заблокований вміст' );
		dialog.done( function() {
			thread.m_bLoading = true;
