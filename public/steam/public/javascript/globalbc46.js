	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Sisältö ei sovi alaikäisille', data['html'], 'Näytä sisältö' );
				...
				{
					ShowAlertDialog('Sisältö ei sovi alaikäisille', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Näytä aikuissisällön varoitus' ).addClass( 'option' );
	...
	if ( $elSource.hasClass( "has_adult_content" ) )
	{
		...
		var $elUnblur = $J( '<div/>' ).click( fnUnblur ).text( 'Poista vain tällaisen sisällön sumennus' ).addClass( 'option' );
		$El.append( $elUnblur );
	}
	else
	{
		...
		var $elAddBlur = $J( '<div/>' ).click( fnAddBlur ).text( 'Sumenna sisältö' ).addClass( 'option' );
		$El.append( $elAddBlur );
	}
...
function ApplyAdultContentPreferencesHelper( e, bGlobalHideAdultContentSex, bGlobalHideAdultContentViolence, bForce )
{
	...
	else
	{
		...
		if ( e.width() > 100 && !e.hasClass( 'ugc_show_warning_image' ) && !e.hasClass( 'dynamiclink_box' ) )
		{
			...
			$elWarning.append( $J( '<span>', { 'text': 'Sisältö ei välttämättä ole soveliasta kaikille käyttäjille'} ) );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Jaa', 'Tilapäivitys näkyy nyt Kaveriesi toiminnassa.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Jaa', 'Tilaviestiä jaettaessa ilmaantui ongelma. Yritä uudelleen myöhemmin.' );
		}
	});
}
