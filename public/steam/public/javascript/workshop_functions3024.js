...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Дата публикации' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'С' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'по' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Дата последнего обновления'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'С' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'по' } ) );
	...
	var dialog = ShowConfirmDialog( 'Сортировать по дате', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Название компании';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Имя (как оно указано в данных банка)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Номер маршрутизации';
		}
		else
		{
			...
			{
				$('bankroutingnumberrow').style.display = 'block';
				$('bankroutingnumberlabel').innerHTML = 'Канадский транзитный номер';
			}
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Пожалуйста, введите название компании.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Пожалуйста, введите имя правильно.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Пожалуйста, введите фамилию правильно.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Пожалуйста, введите адрес правильно.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Пожалуйста, введите город правильно.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Пожалуйста, правильно введите почтовый индекс.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Пожалуйста, введите ваш номер телефона, включая код города.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Сохранено!', response.redirect_url.length != 0 ? 'Ваши контактные данные и адрес сохранены. Поскольку вы указали новую страну пребывания, сейчас вас перенаправят на сторонний сайт, где нужно будет обновить свою налоговую информацию.' : 'Ваш адрес и контактные данные были сохранены. Пожалуйста, предоставьте банковскую и налоговую информацию, если вы еще не сделали этого.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Ошибка', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Ошибка', 'Во время сохранения вашей контактной информации произошла ошибка.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Пожалуйста, введите адрес эл. почты правильно.<br/>';
		}
		...
		{
			errorString += 'Пожалуйста, правильно введите название банка.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Пожалуйста, правильно введите имя владельца банковского счета (должно совпадать с данными банка).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Пожалуйста, правильно введите адрес банка.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Пожалуйста, правильно введите город, в котором находится банк.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Пожалуйста, правильно введите почтовый индекс банка.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Пожалуйста, введите номер вашего банковского счета.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Пожалуйста, введите маршрутный номер банка (9 цифр).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Пожалуйста, введите канадский транзитный номер (9 цифр, начинается с 0).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Пожалуйста, введите IBAN банка.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Пожалуйста, введите SWIFT-код банка.<br/>';
			}
		}
		else
		{
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Управление авторами', 'Загрузка списка друзей' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Управление авторами', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Отмена' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Ошибка', 'Указанный профиль не найден.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Вы уверены, что хотите утвердить этих авторов? Вы не сможете добавлять или удалять их после этого шага.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Пожалуйста, подождите', 'Загрузка списка...' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Выбор поставщиков услуг';
		var strSaveChanges = 'Продолжить';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Ошибка', 'Вознаграждение поставщика услуг не должно превышать 100%.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Ошибка', 'Вы не можете выбрать стороннюю организацию, которая также является обычным участником вашего продукта.' );
				break;
			default:
				ShowAlertDialog( 'Ошибка', 'При обработке вашего запроса произошла ошибка:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
