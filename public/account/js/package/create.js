...
$('.category_id')
    ...
    .on('change', (e) => {
        ...
        $.ajax({
            url: '/api/download/list/category/'+category.val()+'/subcategories',
            success: data => {
                let sub_field = document.querySelector('.subcategory_id');

                sub_field.innerHTML = `<option></option>`

                data.subs.forEach(d => {
                    sub_field.innerHTML += `<option value="${d.id}">${d.title}</option>`
                })

                sub_field.removeAttribute('disabled')
            }
        })
    })
...
document.querySelectorAll('[name="type_feature"]').forEach(input => {
    input.addEventListener('change', (e) => {
        e.preventDefault()
        if(input.value == 0) {
            document.querySelector('#vehicle').style.display = 'block'
        } else {
            document.querySelector('#vehicle').style.display = 'none'
        }
    })
...
$("#formAddPackage").on('submit', (e) => {
    e.preventDefault()
    let form = $("#formAddPackage")
