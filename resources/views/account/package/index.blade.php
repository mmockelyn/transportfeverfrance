...
@section("content")
    ...
    <div class="card shadow-sm">
        <div class="card-header">
            <h3 class="card-title">Liste de mes Packages</h3>
            <div class="card-toolbar">
                <span class="m-2">Nombre de Package: <span class="badge badge-success">{{ $user->downloads()->count() }}</span> </span>
                <a href="{{ route('account.packages.create') }}" class="btn btn-sm btn-light-primary m-2"><i class="far fa-plus-square"></i> Nouveau package</a>
                <a href="{{ route('account.packages.import') }}" class="btn btn-sm btn-light-warning disabled" disabled=""><i class="fas fa-file-import"></i> Importer mes packages</a>
            </div>
        </div>
        <div class="card-body">
            <table class="table table-row-bordered gy-5" id="user_list_package">
                <thead>
                <tr>
                    <th>ID</th>
                    <th>Visuel</th>
                    <th>Titre</th>
                    <th class="w-10">Sources</th>
                    <th class="text-center">Etat</th>
                    <th class="text-end">Actions</th>
                </tr>
                </thead>
                <tbody>
                @foreach($user->downloads()->orderBy('updated_at', 'asc')->get() as $download)
                    <tr>
                        <td>{{ $download->id }}</td>
                        <td>
                            <div class="symbol symbol-120 flex-shrink-0 mr-4">
                                ...
                                @else
                                    <div class="symbol-label" style="background-image: url('https://picsum.photos/120')"></div>
                                @endif
                            </div>
                        </td>
                        <td>
                            <strong>{{ $download->title }}</strong><br>
                            <span class="h6 text-muted">{{ $download->shortDescription }}</span>
                        </td>
                        <td>{{ $download->provider }}</td>
                        <td class="text-center">{{ $download->active }}</td>
                        <td class="text-end">
                            <a href="{{ route('account.packages.show', $download->id) }}" class="btn btn-icon btn-primary" data-bs-toggle="tooltip" title="Voir le package"><i class="fas fa-eye"></i> </a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
...
@section("script")
    ...
    <script src="{{ asset('account/js/package/index.js') }}"></script>
@endsection
